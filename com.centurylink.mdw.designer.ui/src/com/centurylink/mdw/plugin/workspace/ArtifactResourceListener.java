...
public class ArtifactResourceListener implements IResourceChangeListener
{
  ...
  public IFile getTempFile() { return tempFile; }

  public ArtifactResourceListener(WorkflowElement element, ArtifactEditorValueProvider valueProvider, IFile tempFile)
  {
          ...
          display.syncExec(new Runnable()
          {
            public void run()
            {
              ...
              if (getElement() instanceof Activity || getElement() instanceof WorkflowProcess)
              {
                ...
                {
                  Activity activity = (Activity) getElement();
                  // the activity the attribute was set on above may be a holdover from a previously-opened process version
  ...
  private ProcessEditor openProcessEditor(WorkflowProcess processVersion) throws PartInitException
  {
    return (ProcessEditor) MdwPlugin.getActivePage().openEditor(processVersion, "mdw.editors.process");
  }
