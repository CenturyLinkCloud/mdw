...
/**
 * Runs Java Server in a spawned shell and prints output to the dedicated
 * server console implemented as part of this plugin.
 */
public class ServerRunner implements ServerStatusListener
{
  ...
  public IJavaProject getJavaProject() { return javaProject; }
  public void setJavaProject(IJavaProject javaProject) { this.javaProject = javaProject; }
   ...
   * @param display - display for user interface thread, enables async
   */
  ...
  public void start()
  {
    ...
    getConsole().print(msg + PluginUtil.getLineDelimiter(), JavaProcessConsole.LINE_TYPE_STATUS);
  ...
  public void stop()
  {
    ...
    getConsole().print("Stopping " + settings.getContainerName() + " Server" + PluginUtil.getLineDelimiter(), JavaProcessConsole.LINE_TYPE_STATUS);
  }
  ...
  class Runner implements Runnable
  {
    ...
    void setStarting(boolean b) { startingServer = b; }
    ...
    public void run()
    {
      ...
      try
      {
        ...
        if (startingServer)
        {
          ...
          p = rt.exec(command, serverConfig.getEnvironment(true), new File(serverConfig.getCommandDir()));
        }
        else
        {
          p = rt.exec(command, serverConfig.getEnvironment(false), new File(serverConfig.getCommandDir()));
        }


        Reader stdOutReader = new BufferedReader(new InputStreamReader(p.getInputStream()));
        ProcInputHandler stdOut = new ProcInputHandler(stdOutReader, JavaProcessConsole.LINE_TYPE_NORMAL);
        Thread tStdOut = new Thread(stdOut);
        tStdOut.start();

        Reader stdErrReader = new BufferedReader(new InputStreamReader(p.getErrorStream()));
        ProcInputHandler stdErr = new ProcInputHandler(stdErrReader, JavaProcessConsole.LINE_TYPE_ERROR);
  ...
  class ServerRunnerWindowListener implements IWindowListener
  {
    ...
    public void windowClosed(IWorkbenchWindow window)
    {
      ...
      PlatformUI.getWorkbench().removeWindowListener(this); // avoid duplicate notices
      ...
      if (kill)
      {
        ...
        try
        {
          Process p = rt.exec(command, serverConfig.getEnvironment(false), new File(serverConfig.getCommandDir()));
