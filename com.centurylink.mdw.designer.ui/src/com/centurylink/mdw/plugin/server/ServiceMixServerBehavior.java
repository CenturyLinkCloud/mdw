...
public class ServiceMixServerBehavior extends ServerBehaviourDelegate
...
// return;
    ...
    public void stop(boolean force) {
        ...
        serverSettings = getServerSettings();
        if (serverSettings == null) {
            showError("Launch " + getServer().getName(),
                    "Unable to find server settings for: " + getServer().getName());
            return;
        }
    ...
    public boolean isRefreshOutputDirectoryBeforePublish() {
        return getServer().getAttribute(REFRESH_OUTPUT_DIR_BEFORE_PUBLISH, "true")
    ...
    ServerSettings getServerSettings() {
        ...
        ServiceMixRuntime runtime = (ServiceMixRuntime) getServer().getRuntime()
                .loadAdapter(RuntimeDelegate.class, null);
        if (!serverLoc.isEmpty() && runtime != null && runtime.getLocation() != null
            ...
            serverSettings
                    .setJavaOptions(getServer().getAttribute(JAVA_OPTIONS, DEFAULT_JAVA_OPTS));
            ...
            serverSettings
                    .setSuspend(getServer().getAttribute(DEBUG_SUSPEND, DEFAULT_DEBUG_SUSPEND));
        }
    ...
    WorkflowProject getProject() {
        for (IModule module : getServer().getModules()) {
            ...
            if (project != null) {
                WorkflowProject wfProj = WorkflowProjectManager.getInstance()
