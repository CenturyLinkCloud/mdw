...
public class ServiceMixConfigurator extends ServerConfigurator {
    ...
    public void doDeploy(Shell shell) {
        ...
        for (IServer server : ServerCore.getServers()) {
            if (server.getRuntime() != null && server.getRuntime().getRuntimeType() != null
                    && server.getRuntime().getRuntimeType().getId()
                            .startsWith("com.centurylink.server.runtime.servicemix")) {
        ...
        if (matchingServer != null) {
            ...
            MessageDialog.openError(shell, "Server Deploy",
                    "Could not find a ServiceMix server with a module matching "
    ...
    public void doClientShell(Shell shell) {
        try {
            ...
            File karafShellProps = new File(getServerSettings().getServerLoc() + FILE_SEP + "etc"
                    + FILE_SEP + "org.apache.karaf.shell.cfg");
                    ...
                    if (!sshPort.equals(String.valueOf(getServerSettings().getCommandPort()))) {
                        if (!MessageDialog.openConfirm(shell, "SSH Port Mismatch",
                                "The SSH port configured in the MDW Server Settings ("
                                        ...
                                        + karafShellProps + " (" + sshPort
                                        + ").  Continue Client Shell Launch?")) {
                            return;
                        }
                    }
                }
        ...
        try {
            ...
            ServerConsoleSettings consoleSettings = MdwPlugin.getSettings()
                    .getServerConsoleSettings();
            if (ClientShell.Putty == consoleSettings.getClientShell()) {
                ...
                if (exe != null) {
                    if (exe.exists() && exe.isFile())
                        rt.exec(exe + " -ssh -P " + getServerSettings().getCommandPort() + " -l "
                                + getServerSettings().getUser() + " -pw "
                                + getServerSettings().getPassword() + " "
                ...
                // karaf client shell
                rt.exec("cmd /c start client.bat -a " + getServerSettings().getCommandPort()
                        + " -u " + getServerSettings().getUser() + " -p "
                        + getServerSettings().getPassword(), env, smixBinDir);
            }
        }
        catch (Exception ex) {
    ...
    public String[] getEnvironment(boolean debug) {
        ...
        env[1] = "JAVA_OPTS=" + (getServerSettings().getJavaOptions() == null ? ""
