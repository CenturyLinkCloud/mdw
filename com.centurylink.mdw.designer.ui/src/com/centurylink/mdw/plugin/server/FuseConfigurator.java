...
public class FuseConfigurator extends ServiceMixConfigurator
{
  ...
  public void doDeploy(Shell shell)
  {
    ...
    for (IServer server : ServerCore.getServers())
    {
      if (server.getRuntime() != null && server.getRuntime().getRuntimeType() != null
          && server.getRuntime().getRuntimeType().getId().startsWith("com.centurylink.server.runtime.jbossfuse"))
      {
  ...
  public String getStartCommand()
  {
    ...
    if (cmd.equals("karaf.bat") && !new File(fullCmd).exists())
    {
      ...
      karafBat.append("SET KARAF_HOME=").append(getServerSettings().getHome().replace('/', '\\')).append("\r\n");
      karafBat.append("SET KARAF_BASE=").append(getServerSettings().getServerLoc().replace('/', '\\')).append("\r\n");
      ...
      {
        PluginMessages.uiError(ex, "Create Start Cmd", this.getServerSettings().getProject());
      }
    }
