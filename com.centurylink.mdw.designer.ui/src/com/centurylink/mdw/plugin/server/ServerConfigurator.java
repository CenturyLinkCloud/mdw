...
public abstract class ServerConfigurator {
        ...
        public static ServerConfigurator create(ServerSettings serverSettings) {
            if (serverSettings.isWebLogic())
                ...
                throw new IllegalArgumentException(
                        "Unsupported container: " + serverSettings.getContainerName());
        }
    }
    ...
    public abstract void parseServerAdditionalInfo()
    ...
    /**
     * Gets a string array containing the current name/value pairs for the
     * runtime environment.
     */
    ...
    public String getConfigDir() {
        if (getServerSettings().getProject().isCloudProject())
            return getServerSettings().getProject().getSourceProject()
                    .getFolder(new Path("deploy/config")).getLocation().toOSString();
        else {
    ...
    protected String getDebugOptions() {
        return "-Xdebug -Xrunjdwp:transport=dt_socket,server=y,"
                + (getServerSettings().isSuspend() ? "suspend=y," : "suspend=n,") + "address="
    ...
    protected void createCloudBase(IProject project, IProgressMonitor monitor)
        ...
        try {
            ...
            ProjectUpdater updater = new ProjectUpdater(getServerSettings().getProject(),
                    MdwPlugin.getSettings());
            updater.updateMappingTemplates(project.getFolder(new Path(tempLoc + "/deploy/config")),
                    monitor);
        }
        catch (Exception ex) {
            PluginMessages.log(ex);
