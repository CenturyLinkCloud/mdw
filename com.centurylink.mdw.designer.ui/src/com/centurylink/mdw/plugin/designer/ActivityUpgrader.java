  ...
  boolean doUpgrade() throws IOException
  {
    ...
    else if (INVOKE_HETERO.equals(activity.getImplementorClassName()))
    {
      ...
      if (mapAttr != null && !mapAttr.isEmpty())
      {
        ...
        activity.setAttribute(WorkAttributeConstant.PROCESS_MAP, StringHelper.serializeTable(rows));
        updated = true;
      }
    }
        ...
        || CUSTOM_TASK.equals(activity.getImplementorClassName()))
    {
      ...
      if (noticesAttr == null || noticesAttr.isEmpty() || noticesAttr.equals("$DefaultNotices")
      ...
      {
        int colCount = StringHelper.delimiterColumnCount(noticesAttr.substring(0, noticesAttr.indexOf(";")), ",", "\\,")+1;
        ...
        for (String[] row : rows)
        {
          ...
          newRow[0] = row[0];
          newRow[1] = row[1];
          newRow[2] = "0"; // we don't know what version to use, so use latest
        ...
        activity.setAttribute(TaskAttributeConstant.NOTICES, StringHelper.serializeTable(newRows));
      }
      updated = true;
    }
