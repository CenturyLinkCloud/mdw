...
public class ProcessExplorerDropTarget extends ViewerDropAdapter
{
  ...
  /**
   * Workflow element string format (elements delimited by '#'):
   * Process: "Process~MyWorkflowProject^MyPackage v1.0^MyProcess v1.0"
   * ExternalEvent: "ExternalEvent~MyWorkflowProject^MyPackage v1.0^extEvtId"
   * ActivityImpl: "ActivityImpl~MyWorkflowProject^MyPackage v1.0^activityImplId"
  ...
  public boolean isValidDrop(String elementInfo, Object target, int operation)
  {
    if (target instanceof WorkflowElement && ((WorkflowElement)target).getPackage() != null)
    {
      ...
      for (String element : elements)
      {
        ...
        {
          // only allow copies for elements in the same project (exception: single process)
          if (!targetPackage.getProject().getName().equals(workflowProjectName))
          {
          ...
          if (!element.startsWith("Process")
              && !element.startsWith("ExternalEvent")
    ...
    for (String element : elements)
    {
      if (element.startsWith("Process~"))
      {
        ...
        for (WorkflowProcess process : targetPackage.getProcesses())
        {
          ...
          {
            String msg = "Package '" + targetPackage.getName() + "' already contains version "
              + process.getVersionLabel() + " of process '" + process.getName() + "'";
  ...
  private boolean handleDropProcess(String processInfo, WorkflowPackage targetPackage, int operation)
  {
    ...
    if (operation == DND.DROP_COPY)
    {
      ...
      else
      {
        ...
        CopyDialog copyDialog = new CopyDialog(getViewer().getControl().getShell(), existing, processName, version);
  ...
  private boolean handleDropExternalEvent(String externalEventInfo, WorkflowPackage targetPackage, int operation)
  {
    ...
    if (operation == DND.DROP_COPY)
    {
      ...
      CopyDialog copyDialog = new CopyDialog(getViewer().getControl().getShell(), existing, existing.getMessagePattern(), null);
      if (copyDialog.open() == Dialog.OK)
      {
        String newPattern = copyDialog.getNewName();
        ExternalEvent newCopy = new ExternalEvent(existing);
        newCopy.setMessagePattern(newPattern);
    ...
    if (operation == DND.DROP_COPY)
    {
      ...
      if (copyDialog.open() == Dialog.OK)
      {
        ...
        try
        {
          ...
          Constructor<? extends WorkflowAsset> ctor = existing.getClass().getConstructor(new Class<?>[]{existing.getClass()});
