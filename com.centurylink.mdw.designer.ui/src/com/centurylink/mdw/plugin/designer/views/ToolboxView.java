...
public class ToolboxView extends ViewPart implements ITabbedPropertySheetPageContributor, DirtyStateListener
{
  ...
  public void dirtyStateChanged(boolean dirty)
  {
    if (toolboxWrapper.getToolboxSelection() != null)
      actionGroup.getSaveAction().setEnabled(dirty && toolboxWrapper.getToolboxSelection().isUserAuthorized(UserRoleVO.ASSET_DESIGN));
    updateViewTitle();
  }
  ...
  private void setViewTitleTooltip()
  {
    if (toolboxWrapper.isInstance())
      ...
      setTitleToolTip("Toolbox" + (toolboxWrapper.isPopulated() ? " for " + toolboxWrapper.getProject().getName() : ""));
  }
  ...
  public void handleFilter()
  {
    if (isDirty())
    {
      MessageDialog.openWarning(getSite().getShell(), "Please Save", "Please save or abandon outstanding changes before filtering");
      return;
    }
  ...
  public void handleSave()
  {
    ...
    if (save)
    {
      ...
      int res = PluginMessages.uiList(getSite().getShell(), "The following modified activity implementors will be saved", "Save Implementors", dirtyImplNames);
  ...
  public void handleNew()
  {
    if (isDirty())
    {
      MessageDialog.openWarning(getSite().getShell(), "Please Save", "Please save or abandon outstanding changes before creating a new implementor");
      return;
    }
    else
    {
  ...
  public void handleDiscover()
  {
    if (isDirty())
    {
      MessageDialog.openWarning(getSite().getShell(), "Please Save", "Please save or abandon outstanding changes before discovering new assets");
      return;
    }
    else
    {
  ...
  public void handleDelete()
  {
    ...
    if (isDirty())
    {
      MessageDialog.openWarning(getSite().getShell(), "Please Save", "Please save or abandon outstanding changes before deleting");
      return;
    }
    else
    {
