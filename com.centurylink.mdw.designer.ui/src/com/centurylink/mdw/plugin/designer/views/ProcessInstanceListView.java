...
public class ProcessInstanceListView extends ViewPart
{
  ...
  public void createPartControl(Composite parent)
  {
    ...
    PlatformUI.getWorkbench().getHelpSystem().setHelp(parent, MdwPlugin.getPluginId() + ".process_instance_help");
  }
  ...
  public void filter()
  {
    ...
    ProcessInstanceFilterDialog filterDialog = new ProcessInstanceFilterDialog(getSite().getShell(), processVersion, contentProvider.getFilter());
  ...
  private WorkflowProcess getProcessVersion(WorkflowProcess processVersion, String version)
  {
    if (version.length() == 0)
    {
      ...
      pv.getProcessVO().setProcessId(0L); // indicates exclude version from criteria
  ...
  private void refreshProcesses()
  {
    BusyIndicator.showWhile(getSite().getShell().getDisplay(), new Runnable()
    {
      public void run()
      {
        ...
        if (workflowProject != null)
        {
          List<ProcessVO> processVOs = workflowProject.getDataAccess().getProcesses(false);
    ...
    BusyIndicator.showWhile(getSite().getShell().getDisplay(), new Runnable()
    {
      public void run()
      {
        ...
        if (editor instanceof ProcessEditor)
        {
          ...
          PanelBusyIndicator pbi = new PanelBusyIndicator(getSite().getShell().getDisplay(), canvas);
          try
          {
