...
public class ProcessExplorerContentProvider implements ITreeContentProvider, ElementChangeListener {
    ...
    public Object[] getChildren(Object parentElement) {
        if (parentElement instanceof WorkflowProject) {
                        ...
                        public void run(IProgressMonitor monitor)
                            ...
                            try {
                                ...
                                if (workflowProject.getDataAccess() != null) {
                                    ...
                                    workflowProject
                                            .getArchivedUserVisiblePackagesFolder(progressMonitor);
            ...
            List<WorkflowPackage> topLevelPackages = workflowProject
            ...
            if (packageVersion.isArchived() && packageVersion.hasDescendantPackageVersions()) {
                return packageVersion.getDescendantPackageVersions()
                        .toArray(new WorkflowPackage[0]);
            }
            else {
                ...
                if (prefsStore
                        .getBoolean(PreferenceConstants.PREFS_SHOW_ACTIVITY_IMPLEMENTORS_IN_PEX))
            ...
            if (processVersion.hasDescendantProcessVersions()) {
                return processVersion.getDescendantProcessVersions()
                        .toArray(new WorkflowProcess[0]);
            }
            else {
                return EMPTY_ARRAY;
            }
    ...
    public boolean hasChildren(Object element) {
        if (element instanceof WorkflowProject) {
            ...
            WorkflowPackage packageVersion = (WorkflowPackage) element;
            IPreferenceStore prefsStore = MdwPlugin.getDefault().getPreferenceStore();
            return packageVersion.hasDescendantPackageVersions() || (packageVersion.hasProcesses()
                    && !prefsStore.getBoolean(PreferenceConstants.PREFS_FILTER_PROCESSES_IN_PEX))
                    || (packageVersion.hasAssets() && !prefsStore
                            .getBoolean(PreferenceConstants.PREFS_FILTER_WORKFLOW_ASSETS_IN_PEX))
                    || (packageVersion.hasExternalEvents() && !prefsStore
                            .getBoolean(PreferenceConstants.PREFS_FILTER_EVENT_HANDLERS_IN_PEX))
                    || (packageVersion.hasActivityImpls() && prefsStore.getBoolean(
                            PreferenceConstants.PREFS_SHOW_ACTIVITY_IMPLEMENTORS_IN_PEX))
                    || (packageVersion.hasTaskTemplates() && !prefsStore
                            .getBoolean(PreferenceConstants.PREFS_FILTER_TASK_TEMPLATES_IN_PEX))
        ...
        if (ece.getChangeType().equals(ChangeType.ELEMENT_CREATE)) {
            ...
            if (parent != null) {
                if (ece.getElement() instanceof ActivityImpl)
                    treeViewer.refresh(); // impl can be associated with
                                          // multiple packages
                else
                    treeViewer.refresh(parent, true);
            }
            ...
            if (ece.getElement() instanceof WorkflowPackage)
                treeViewer.refresh(
                        ece.getElement().getProject().getArchivedUserVisiblePackagesFolder(), true);
        }
