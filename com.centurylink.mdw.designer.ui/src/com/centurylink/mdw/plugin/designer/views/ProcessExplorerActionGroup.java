...
public class ProcessExplorerActionGroup extends ActionGroup
{
  ...
  public ProcessExplorerActionGroup(ProcessExplorerView view)
  {
    ...
    newCloudProjectAction = createNewAction(WorkflowProject.class, "Local Cloud Project", "cloud_project.gif");
    ...
    newGeneralActivityAction = createNewAction(Activity.class, "General Activity", "genact_wiz.gif");
    newStartActivityAction = createNewAction(StartActivity.class, "Start Activity", "startact_wiz.gif");
    newAdapterActivityAction = createNewAction(AdapterActivity.class, "Adapter Activity", "adaptact_wiz.gif");
    newEvaluatorActivityAction = createNewAction(EvaluatorActivity.class, "Evaluator Activity", "evalact_wiz.gif");
    newEventHandlerAction = createNewAction(ExternalEvent.class, "External Event Handler", "extevent_wiz.gif");
    newCamelProcessHandlerAction = createNewAction(ExternalEvent.CamelProcessLaunch.class, "Camel Process Launch Handler", "camel.gif");
    newCamelNotifyHandlerAction = createNewAction(ExternalEvent.CamelEventNotify.class, "Camel Event Notify Handler", "camel.gif");
        ...
        if (importAttributesApplies(selection))
        {
          ...
          if (!importAttrsActions.isEmpty())
          {
            MenuManager attributesMenu = new MenuManager("Attributes",
                MdwPlugin.getImageDescriptor("icons/attribute.gif"), MdwMenuManager.MDW_MENU_PREFIX + "menu.import.attributes");
    ...
    return new ActionGroup()
    {
      ...
      public void fillContextMenu(IMenuManager menu)
      {
        ...
        if (exportAttributesApplies(selection))
        {
          ...
          {
            MenuManager attributesMenu = new MenuManager("Attributes",
                MdwPlugin.getImageDescriptor("icons/attribute.gif"), MdwMenuManager.MDW_MENU_PREFIX + "menu.export.attributes");
            attributesMenu.removeAll();
  ...
  private IAction createPasteAction()
  {
    IAction action = new Action()
    {
      public void run()
      {
        ...
        actionHandler.paste(view.getClipboard(), view.getDropTarget(), (WorkflowElement)getSelection().getFirstElement());
      }
    };
    ISharedImages sharedImages = PlatformUI.getWorkbench().getSharedImages();
  ...
  private IAction createDeleteAction()
  {
    ...
    IAction action = new Action()
    {
      public void run()
      {
        ...
        if (includesArchived) // TODO archived elements are not being listened to
    ...
    IAction action = new Action()
    {
      public void run()
      {
        if (!importProjectApplies(getSelection()))
          return;
        actionHandler.imporT(WorkflowProject.class, (WorkflowElement)getSelection().getFirstElement());
      }
    };
  ...
  private IAction createImportPackageAction()
  {
    IAction action = new Action()
    {
      public void run()
      {
        ...
        boolean authorized = (workflowProject.isFilePersist() && !workflowProject.isRemote())
            || workflowProject.getDesignerDataModel().userHasRoleInAnyGroup(UserRoleVO.PROCESS_DESIGN);
        if (!authorized)
        {
          MessageDialog.openError(getViewSite().getShell(), "Package Import", "You are not authorized to import into project: " + workflowProject.getName());
          return;
        }
    ...
    IAction action = new Action()
    {
      public void run()
      {
        if (!importProcessApplies(getSelection()))
          return;
        actionHandler.imporT(WorkflowProcess.class, (WorkflowElement)getSelection().getFirstElement());
      }
    };
      ...
      public void run()
      {
        ...
        if (!workflowProject.isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_DESIGN))
        {
          MessageDialog.openError(getViewSite().getShell(), "Task Template Import",
              "You are not authorized to import into project: " + workflowProject.getName());
          return;
        }
    ...
    IAction action = new Action()
    {
      public void run()
      {
        if (!importWorkflowAssetApplies(getSelection()))
          return;
        actionHandler.imporT(WorkflowAsset.class, (WorkflowElement)getSelection().getFirstElement());
      }
    };
    ...
    IAction action = new Action()
    {
      public void run()
      {
        ...
        actionHandler.launchSwing(workflowProject, title,
            formName == null ? null : listenerPackage + formName, handlerPkg + dataInit);
      }
    };
  ...
  private IAction createRunFromStartPageAction()
  {
    IAction action = new Action()
    {
      public void run()
      {
        ...
        if (element instanceof AutomatedTestSuite || element instanceof AutomatedTestCase)
          actionHandler.test(getSelection());
        else if (element instanceof File && ((File)element).getParent() instanceof AutomatedTestCase &&
            TestCase.LEGACY_TEST_CASE_FILENAMES.values().contains(((File)element).getName()))
          actionHandler.test(getSelection());
        else
  ...
  private IAction createRefreshCachesAction()
  {
    IAction action = new Action()
    {
      public void run()
      {
        actionHandler.serverAction((WorkflowElement)getSelection().getFirstElement(), WorkflowElementActionHandler.REFRESH_CACHES);
      }
    };
  ...
  private IAction createStubServerAction()
  {
    IAction action = new Action("Stub Server", Action.AS_CHECK_BOX)
    {
      public void run()
      {
        ...
        project.fireElementChangeEvent(ChangeType.STATUS_CHANGE, project.getDesignerProxy().isStubServerRunning());
      }
    };
  ...
  private IAction createNewAction(final Class<? extends WorkflowElement> elementClass, String label, String icon)
  {
    IAction action = new Action()
    {
      public void run()
      {
        if (!createApplies(elementClass, getSelection()))
          return;
        actionHandler.create(elementClass, (WorkflowElement)getSelection().getFirstElement());
      }
    };
  ...
  {
    if (getContext() == null || WorkflowProjectManager.getInstance().getWorkflowProjects().isEmpty())
  ...
  public boolean showPropertiesApplies(IStructuredSelection selection)
  {
    ...
    if (element instanceof WorkflowProject
        || element instanceof WorkflowPackage
        || element instanceof WorkflowProcess
        || element instanceof ExternalEvent
        || element instanceof WorkflowAsset
        || element instanceof AutomatedTestCase)
    {
      return true;
    }
  ...
  public boolean incrementVersionApplies(IStructuredSelection selection)
  {
    ...
    if (packageVersion.isDefaultPackage() || packageVersion.isArchived() || !packageVersion.isUserAuthorized(UserRoleVO.ASSET_DESIGN))
      return false;
  ...
  {
    boolean allowArchived = MdwPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.PREFS_ALLOW_DELETE_ARCHIVED_PROCESSES);
  ...
  {
    boolean deleteArchived = MdwPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.PREFS_ALLOW_DELETE_ARCHIVED_PROCESSES);
    ...
    for (Object item : selection.toList())
    {
      ...
      if (!(workflowElement instanceof WorkflowProject) && !workflowElement.isUserAuthorized(UserRoleVO.ASSET_DESIGN))
      {
        return false;
      }
    }
  ...
  public boolean importPackageApplies(IStructuredSelection selection)
  {
    ...
    if (project.isFilePersist())
    {
      ...
      {
        if (project.isGitVcs())
          return false; // not applicable for remote Git
      }
      else
      {
  ...
  public boolean importAttributesApplies(IStructuredSelection selection)
  {
    ...
    {
      WorkflowPackage pkg = (WorkflowPackage) element;
      return !pkg.isDefaultPackage() && pkg.getProject().getDataAccess().getDesignerDataModel().userHasRole(pkg.getGroup(), UserRoleVO.ASSET_DESIGN);
    }
    else if (element instanceof WorkflowProcess)
    {
      WorkflowProcess proc = (WorkflowProcess) element;
      return !proc.isInDefaultPackage() && proc.isInRuleSet() && proc.getPackage().isUserAuthorized(UserRoleVO.ASSET_DESIGN);
    }
  ...
  {
    return exportProjectApplies(selection)
        || exportPackageApplies(selection)
        || exportProcessApplies(selection)
        || exportWorkflowAssetApplies(selection)
        || exportAttributesApplies(selection)
        || exportTaskTemplatesApplies(selection);
  }
  ...
  public boolean runTestsApplies(IStructuredSelection selection)
  {
    if (selection.size() == 1 && selection.getFirstElement() instanceof WorkflowElement)
    {
      WorkflowElement element = (WorkflowElement) selection.getFirstElement();
      if (element instanceof WorkflowProject)
        return ((WorkflowProject)element).isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_EXECUTION);
  ...
  public boolean runApplies(IStructuredSelection selection)
  {
    if (selection.size() > 1)
    {
      ...
      return ((AutomatedTestCase)selection.getFirstElement()).getProject().isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_EXECUTION);
    }
    ...
    {
      if (workflowElement.getProject() == null || !workflowElement.getProject().isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_EXECUTION))
        return false;
  ...
  public boolean searchApplies(IStructuredSelection selection)
  {
    return selection.size() == 1
      && (selection.getFirstElement() instanceof WorkflowProject || selection.getFirstElement() instanceof WorkflowPackage);
  }
  ...
  public boolean createApplies(Class<? extends WorkflowElement> elementClass, IStructuredSelection selection)
  {
    if (selection.size() > 1)
      return false;
    ...
    if (selection.size() == 1)
    {
      ...
      {
        WorkflowElement workflowElement = (WorkflowElement) selection.getFirstElement();
        if (!workflowElement.getProject().isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_DESIGN))
          return false;
      }
      return true;
    }
  ...
  public boolean refreshApplies(IStructuredSelection selection)
  {
    ...
    if (selection.size() == 1
        && (selection.getFirstElement() instanceof AutomatedTestSuite
  ...
  public boolean serverActionApplies(IStructuredSelection selection)
  {
    if (selection.size() == 1)
    {
      ...
      {
        WorkflowProject project = (WorkflowProject) selection.getFirstElement();
        if (project.isUserAuthorizedForSystemAdmin() || project.isUserAuthorizedInAnyGroup(UserRoleVO.PROCESS_EXECUTION))
          return true;
      }
    }
    return false;
  ...
  {
    return formatFunctionTestResultsApplies(selection) || formatLoadTestResultsApplies(selection);
  }
