 ...
 * Wraps a PackageVO.
 */
public class WorkflowPackage extends WorkflowElement
        implements Versionable, AttributeHolder, Comparable<WorkflowPackage> {
    ...
    public void setPackageVO(PackageVO pkgVO) {
        this.packageVO = pkgVO;
    }
    ...
    public List<WorkflowPackage> getDescendantPackageVersions() {
        return descendantPackageVersions;
    }

    public void setDescendantPackageVersions(List<WorkflowPackage> dpvs) {
        descendantPackageVersions = dpvs;
    }
    ...
    public List<ActivityImpl> getActivityImpls() {
        return activityImpls;
    }

    public void setActivityImpls(List<ActivityImpl> activityImpls) {
        this.activityImpls = activityImpls;
    }
    ...
    public WorkflowPackage getTopLevelVersion() {
        return topLevelVersion;
    }

    public void setTopLevelVersion(WorkflowPackage topLevelVersion) {
    ...
    public Folder getArchivedFolder() {
        return archivedFolder;
    }

    public void setArchivedFolder(Folder folder) {
        this.archivedFolder = folder;
    }
    ...
    public WorkflowPackage(WorkflowProject wfProject, PackageVO packageVO) {
        ...
        if (packageVO.getSchemaVersion() == 0)
            packageVO.setSchemaVersion(
                    wfProject.getDesignerProxy().getPluginDataAccess().getSchemaVersion());
    }
    ...
    public boolean isVisible() {
        boolean permission = StringHelper.isEmpty(getGroup())
                || getGroup().equals(UserGroupVO.COMMON_GROUP)
                || getDesignerDataModel().belongsToGroup(getGroup());
        if (permission) {
            // check for VCS prefix filters
            if (getProject().isFilePersist()
                    && getProject().getMdwVcsRepository().getPkgPrefixes() != null) {
                // further restrict
