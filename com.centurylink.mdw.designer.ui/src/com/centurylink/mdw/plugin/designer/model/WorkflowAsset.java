...
public class WorkflowAsset extends WorkflowElement implements AttributeHolder, Versionable, Comparable<WorkflowAsset>, IResourceChangeListener
{
  private RuleSetVO ruleSetVO;
  public RuleSetVO getRuleSetVO() { return ruleSetVO; }
  protected void setRuleSetVO(RuleSetVO ruleSetVO) { this.ruleSetVO = ruleSetVO; }
  ...
  public List<WorkflowAsset> getAllVersions()
  {
    ...
    for (WorkflowAsset asset : getProject().getAllWorkflowAssets())
    {
      if (this.getName().equals(asset.getName())
          && ((this.getLanguage() == null && asset.getLanguage() == null) || this.getLanguage().equals(asset.getLanguage()))
          && (this.isInDefaultPackage() || asset.isInDefaultPackage() || this.getPackage().getName().equals(asset.getPackage().getName()))
  ...
  public IFile getTempFile(IFolder folder)
  {
    ...
    if (isInDefaultPackage() || !isLatestVersion())
      filename = filename.substring(0, filename.lastIndexOf('.')) + "_v" + getVersion() + filename.substring(filename.lastIndexOf('.'));
    return folder.getFile(filename);
  }

  private IEditorPart assetFileEditor;
  public IEditorPart getFileEditor() { return assetFileEditor; }
  ...
  public void openFile(IProgressMonitor monitor)
  {
    ...
    try
    {
      ...
      if (assetFileEditor != null)
      {
        assetFileEditor = IDE.openEditor(activePage, file, true);  // activate existing editor
      }
      else
      {
        load();
      ...
      if (file.exists())
      {
        ...
        if (assetFileEditor == null)
        {
          ...
          catch (org.eclipse.core.internal.resources.ResourceException ex)
          {
            if (isForceExternalEditor())
            {
              // assume this is because the temp file is already open in an external editor; reactivate
              PluginMessages.log(ex);
  ...
  public void load()
  {
    if (MdwPlugin.isUiThread())
    {
      BusyIndicator.showWhile(MdwPlugin.getDisplay(), new Runnable()
      {
        public void run()
        {
          WorkflowAsset loaded = getProject().getDesignerProxy().loadWorkflowAsset(WorkflowAsset.this);
  ...
  public byte[] getFileContent()
  {
    ...
    {
      fileContent = getContent() == null ? getDefaultContent().getBytes() : getContent().getBytes();
    }
    return fileContent;
  }
        ...
        if (fileEditor.isDirty())
        {
          ...
          if (!isUserAuthorized(UserRoleVO.ASSET_DESIGN))
          {
            shell.getDisplay().asyncExec(new Runnable()
            {
              public void run()
              {
                MessageDialog.openWarning(shell, "Won't Update " + getTitle(), "You're not authorized to update '" + getLabel() + "'\nin workflow project '" + getProject().getName() + "'.");
              }
            });
          }
              ...
              shell.getDisplay().asyncExec(new Runnable()
              {
                public void run()
                {
                  if (getLockingUser() == null)
                  {
                    ...
                    boolean lock = MessageDialog.openQuestion(shell, "Lock " + getTitle(), "Lock resource '" + getLabel() + "' to user " +
                        getProject().getUser().getUsername() + "?");
                    if (lock)
                    {
                      try
                      {
                        getProject().getDesignerProxy().toggleWorkflowAssetLock(WorkflowAsset.this, true);
                        setLockingUser(getProject().getUser().getUsername());
                        setModifyDate(new Date());
                        fireElementChangeEvent(ChangeType.PROPERTIES_CHANGE, null);
                      }
                      catch (Exception ex)
                      {
                        PluginMessages.uiError(shell, ex, "Lock Resource", getProject());
                      }
                    }
                    else
                    {
                      MessageDialog.openWarning(shell, "Won't Update " + getTitle(), "Resource '" + getLabel() + "' is not locked by you, so updates will not be persisted.");
                    }
                  }
                  else
                  {
  ...
  /**
   * Change listener so we'll know when the resource is changed in the workspace.
   */
              ...
              else
              {
                try
                {
                  ...
                  if (versionIncrement != Increment.Overwrite)
                  {
                    ...
                    {
                      // deregister since save never happened
