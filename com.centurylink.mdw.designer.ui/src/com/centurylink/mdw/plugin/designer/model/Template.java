...
public class Template extends WorkflowAsset {
    ...
    public void runWith(final String inputPath, final String outputLocation,
            final String velocityPropFile, final String velocityToolboxFile) {
        try {
            ...
            final org.apache.velocity.Template velocityTemplate = engine
                    .getTemplate(getTempFileName());
            ...
            if (input.isDirectory()) {
                        ...
                        for (java.io.File inputFile : inputFiles) {
                            if (!inputFile.isDirectory()) {
                                String outputFile = outputLocation + "/"
                                        + getOutputFileName(inputFile.getName());
                                monitor.subTask(inputFile.getName());
                                InputStream inputStream = null;
                                try {
                                    inputStream = new FileInputStream(inputFile);
                                    processInput(inputStream, outputLocation, outputFile,
                                            velocityContext, velocityTemplate);
                                }
                                catch (Exception ex) {
                ...
                processInput(new FileInputStream(inputPath), outputLocation, outputFile,
                        velocityContext, velocityTemplate);
    ...
    private void processInput(InputStream input, String outputLocation, String outputFileName,
            VelocityContext velocityContext, org.apache.velocity.Template velocityTemplate)
            throws IOException {
        ...
        for (Object object : props.keySet()) {
            if (props.get(object).toString().toUpperCase().equals("TRUE")
                ...
                velocityContext.put(object.toString(),
                        Boolean.valueOf(props.get(object).toString()));
            }
            else {
