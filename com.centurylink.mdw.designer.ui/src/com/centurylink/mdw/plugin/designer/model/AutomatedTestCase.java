...
public class AutomatedTestCase extends WorkflowAsset
{
      ...
      if (testCaseDir != null)
      {
        ...
        if (testCaseFiles != null)
        {
          for (File file : testCaseDir.listFiles())
          {
            ...
            else if (!TestCase.LEGACY_TEST_CASE_FILENAMES.values().contains(file.getName()) && !file.isDirectory())
            {
              IFolder testCaseFolder = getProject().getOldTestCasesFolder().getFolder(getName());
              files.add(new com.centurylink.mdw.plugin.designer.model.File(getProject(), this, testCaseFolder.getFile(file.getName())));
            }
          }
  ...
  public void setTestSuite(AutomatedTestSuite suite) { this.testSuite = suite; }

  private int runCount;
  public int getRunCount() { return runCount; }
  public void setRunCount(int count) { this.runCount = count; }

  private List<LegacyExpectedResults> legacyExpectedResults;
  public List<LegacyExpectedResults> getLegacyExpectedResults() { return legacyExpectedResults; }

  /**
   * Relies on the convention that results have the same name and reside in the same
   * workflow package as the test case asset.
   */
  public AutomatedTestResults getExpectedResults()
  {
    ...
    return (AutomatedTestResults) getProject().getAsset(getPackage().getName() + "/" + assetName);
  }
   ...
   */
  public AutomatedTestCase(RuleSetVO ruleSet, WorkflowPackage workflowPackage, TestCase testCase)
  {
   ...
   */
  public AutomatedTestCase(WorkflowProject workflowProject, AutomatedTestSuite testSuite, TestCase testCase)
  {
    legacy = true;
  ...
  public File getResultsDir()
  {
    if (isLegacy())
      ...
      return new File(getProject().getTestResultsDir(getType()) + "/" + getPackage().getName());
  }
  ...
  public String getLanguage()
  {
    if (isLegacy())
      ...
      return testCase.getLanguage().equals(TestCase.LANGUAGE_GROOVY) ? RuleSetVO.TEST : RuleSetVO.FEATURE;
  }
  ...
  {
    if (isLegacy())
      return getProject().getOldTestCasesFolder().getFolder(getName()).getFile(testCase.getCommandsFileName());
    else
      return getProject().getProjectFolder(getPackage().getName().replace('.', '/')).getFile(getName());
  }

  public boolean isGroovy()
  {
    return TestCase.LANGUAGE_GROOVY.equals(getLanguage()) || RuleSetVO.TEST.equals(getLanguage());
  }

  public boolean isGherkin()
  {
    return TestCase.LANGUAGE_GHERKIN.equals(getLanguage()) || RuleSetVO.FEATURE.equals(getLanguage());
  }
