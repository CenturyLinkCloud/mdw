...
public class ViewLayout extends WorkflowElement
{
  ...
  public ViewLayout(IFile file)
  {
    ...
    try
    {
      ...
      viewDocTemplate = ViewDocument.Factory.parse(new URL(docUrl), Compatibility.namespaceOptions());

      // task variable mappings
      DesignerProxy designerProxy = getProject() == null ? null : getProject().getDesignerProxy();
      if (designerProxy == null || designerProxy.getPluginDataAccess() == null)
        taskVariableMappings =  new HashMap<String,List<String>>();
      else
        taskVariableMappings = designerProxy.getPluginDataAccess().getTaskVariableMappings();
    }
    catch (Exception ex)
    {
  ...
  protected ViewDocument getViewDocTemplate() { return viewDocTemplate; }
        ...
        if (listElem != null)
        {
          ...
          for (UiColumn uiColumn : uiList.getColumnList())
          {
            if (!existingAttrs.contains(uiColumn.getAttribute()))
              options.add(uiColumn.getAttribute());  // not already used
          }
        ...
        if (filterElem != null)
        {
          ...
          for (UiField uiField : uiFilter.getFieldList())
          {
            if (!existingAttrs.contains(uiField.getAttribute()))
              options.add(uiField.getAttribute());  // not already used
          }
          return options.toArray(new String[0]);
        }
      }
        ...
        if (detailElem != null)
        {
          ...
          for (UiRow uiRow : uiDetail.getRowList())
          {
            if (!existingAttrs.contains(uiRow.getAttribute()))
              options.add(uiRow.getAttribute());  // not already used
          }
          return options.toArray(new String[0]);
        }
      }
  ...
  private Map<String,List<String>> removeExistingVariables(List<String> existingAttrs, Map<String,List<String>> varMap)
  {
    // remove variables that are already tied to attributes
    Map<String,List<String>> weeded = new TreeMap<String,List<String>>(PluginUtil.getStringComparator());
  ...
  {
    return false;  // actually controlled by WorkflowAsset (which is disconnected)
  }
