...
public class DesignerPerspective implements IPerspectiveFactory {
    ...
    public static IWorkbenchPage showPerspective(IWorkbenchWindow activeWindow)
            throws WorkbenchException {
        if (MdwPlugin.isRcp())
            return PlatformUI.getWorkbench().showPerspective("MDWDesignerRCP.perspective",
                    activeWindow);
        else
            return PlatformUI.getWorkbench().showPerspective("mdw.perspectives.designer",
                    activeWindow);
    }

    public static void showPerspectiveAndSelectProjectPreferences(IWorkbenchWindow activeWindow,
        ...
        try {
            ...
            if (processExplorer != null) {
                ...
                PropertySheet propSheet = (PropertySheet) page
                        .showView("org.eclipse.ui.views.PropertySheet");
                if (propSheet != null) {
                    TabbedPropertySheetPage tabbedPage = (TabbedPropertySheetPage) propSheet
                            .getCurrentPage();
                    if (tabbedPage != null)
    ...
    public static void promptForShowPerspective(IWorkbenchWindow activeWindow,
        ...
        if (!"mdw.perspectives.designer".equals(pd.getId())) {
            ...
            String switchPref = MdwPlugin
                    .getStringPref(PreferenceConstants.PREFS_SWITCH_TO_DESIGNER_PERSPECTIVE);
            if (switchPref == null || switchPref.length() == 0) {
                ...
                MessageDialogWithToggle mdwt = MessageDialogWithToggle.openYesNoQuestion(
                        activeWindow.getShell(), "Switch Perspective", message, toggleMessage,
                        false, MdwPlugin.getDefault().getPreferenceStore(),
