...
public class ImportPackageSelectPage extends WizardPage {
    ...
    public ImportPackageSelectPage() {
        ...
        setDescription(
                "Choose the workflow package containing assets to import into your workspace.");
    }
    ...
    private void createUpgradeAssetsComposite(Composite parent) {
        ...
        upgradeAssetsCheckbox.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ((ImportPackageWizard) getWizard()).upgradeAssets = upgradeAssetsCheckbox
                        .getSelection();
            }
        });
        ...
        upgradeAssetsLink.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                String href = "/" + MdwPlugin.getPluginId()
                        + "/help/doc/upgradeAssetsDuringImport.html";
                ...
                if (content != null) {
                    try {
                        if (content.trim().startsWith("{")) {
                            ...
                            List<PackageVO> pkgs = importer.importPackages(content);
                            if (pkgs.size() != 1)
                                throw new JSONException("Invalid package count: " + pkgs.size());
                            PackageVO pkg = pkgs.get(0);
                            return pkg.getName() + " v" + pkg.getVersionString();
                        }
                        else {
                            ProcessDefinitionDocument doc = ProcessDefinitionDocument.Factory
                                    .parse(content, Compatibility.namespaceOptions());
                            MDWProcessDefinition def = doc.getProcessDefinition();
                            return def.getPackageName() + " v" + def.getPackageVersion() + " ("
    ...
    class ViewCheckStateProvider implements ICheckStateProvider {
        ...
        public boolean isGrayed(Object element) {
            if (element instanceof Folder) {
                FileCollector collector = new FileCollector((Folder) element);
                return collector.getSelectedDescendantFiles().size() > 0
                        && collector.getSelectedDescendantFiles().size() < collector
                                .getDescendantFiles().size();
            }
