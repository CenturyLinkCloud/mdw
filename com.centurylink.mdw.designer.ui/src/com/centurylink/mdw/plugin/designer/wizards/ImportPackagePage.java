...
public class ImportPackagePage extends WizardPage
{
  ...
  private void createImportControls(Composite parent, int ncol)
  {
    ...
    link.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent e)
      {
        PreferenceDialog pref = PreferencesUtil.createPreferenceDialogOn(getShell(), UrlsPreferencePage.PREF_PAGE_ID, null, null);
        if (pref != null)
        {
          pref.open();
          String discUrl = MdwPlugin.getSettings().getDiscoveryUrl();
  ...
  boolean isPageValid()
  {
    if (getProject() == null)
      return false;
    if (!getProject().isFilePersist() && !getProject().getDesignerDataModel().userHasRoleInAnyGroup(UserRoleVO.PROCESS_DESIGN))
      return false;
    if (isDiscovery())
    {
  ...
  public IStatus[] getStatuses()
  {
    ...
    else if (!getProject().isFilePersist() && !getProject().getDesignerDataModel().userHasRoleInAnyGroup(UserRoleVO.PROCESS_DESIGN))
  ...
  public Folder populateTopFolder(String discoveryUrl, boolean latestVersionsOnly, IProgressMonitor progressMonitor) throws Exception
  {
    Folder folder = null;
  ...
  /**
   * Not foolproof since it relies on asset XML naming convention.
   * returns emptyFolders to be pruned.
   */
