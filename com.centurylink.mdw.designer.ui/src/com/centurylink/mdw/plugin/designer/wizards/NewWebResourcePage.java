...
public class NewWebResourcePage extends WorkflowAssetPage
{
  ...
  private void createTextBasedControls(Composite parent, int ncol)
  {
    ...
    textBasedTypeCombo.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent e)
      {
        ResourceType resourceType = ResourceType.valueOf(textBasedTypeCombo.getText().trim());
  ...
  boolean isPageValid()
  {
    return getProject() != null
        ...
        && ((textBasedRadio.getSelection() && checkString(textBasedTypeCombo.getText().trim())
            || (binaryRadio.getSelection() && checkString(binaryTypeCombo.getText().trim()))))
