...
public class WorkflowAssetPage extends WizardPage {
    ...
    public WorkflowAssetPage(WorkflowAsset workflowAsset) {
        ...
        setDescription(
                "Enter the info for your new " + workflowAsset.getTitle().toLowerCase() + ".");
    }
    ...
    protected void createDocumentControls(Composite parent, int ncol) {
        ...
        if (!getWorkflowAsset().isBinary() || hasTemplateOptions()) {
            ...
            generateNewButton
                    .setText("Generate " + (hasTemplateOptions() ? "from Template" : "Blank"));
        ...
        importFileBrowseButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                dlg.setFilterExtensions(
                        new String[] { "*" + getWorkflowAsset().getExtension(), "*.*" });
                String path = dlg.open();
                if (path != null) {
                    importFileText.setText(path);
                    if (nameTextField.getText().trim().isEmpty())
                        nameTextField.setText(path.substring(
    ...
    boolean isPageValid() {
        return (getProject() != null && checkStringDisallowChars(getWorkflowAsset().getName(), "/")
    ...
    public IStatus[] getStatuses() {
        ...
        if (getProject() == null)
            ...
            msg = "You're not authorized to add a new " + getWorkflowAsset().getName().toLowerCase()
                    + " to this workflow package.";
