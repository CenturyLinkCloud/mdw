...
public abstract class WorkflowAssetWizard extends Wizard implements INewWizard {
    ...
    public WorkflowAssetPage getWorkflowAssetPage() {
        return workflowAssetPage;
    }
    ...
    public void setImportFile(boolean b) {
        this.importFile = b;
    }
    ...
    public void setImportFilePath(String s) {
        this.importFilePath = s;
    }
    ...
    public boolean performFinish() {
        ...
        if (isImportFile()) {
            try {
                ...
                PluginMessages.uiError(getShell(), ex, "Create " + workflowAsset.getTitle(),
            ...
            try {
                ...
                if (docTemplate != null) {
                    ...
                    else if (workflowAsset.getLanguage().equals(RuleSetVO.FACELET)
                            || workflowAsset.getLanguage().equals(RuleSetVO.HTML))
        ...
        try {
            ...
            PluginMessages.uiError(getShell(), ex, "New " + workflowAsset.getTitle(),
                    workflowAsset.getProject());
            return false;
        }
        ...
        if (designerProxy.getRunnerStatus().equals(RunnerStatus.SUCCESS)) {
            ...
            DesignerPerspective.promptForShowPerspective(
                    PlatformUI.getWorkbench().getActiveWorkbenchWindow(), workflowAsset);
    ...
    /**
     * Override to present a list of template options. Default behavior is to
     * list the files discovered via getTemplateLocation().
     */
    public List<String> getTemplateOptions() {
        ...
        if (templateLoc == null) {
            ...
            try {
                ...
                PluginMessages.uiError(getShell(), ex, "Load Templates",
                        getWorkflowAsset().getProject());
            }
    ...
    public DocumentTemplate getNewDocTemplate() {
        if (getTemplateLocation() == null) {
            ...
            return new DocumentTemplate(name, getWorkflowAsset().getExtension(),
                    getTemplateLocation());
        }
    }
}