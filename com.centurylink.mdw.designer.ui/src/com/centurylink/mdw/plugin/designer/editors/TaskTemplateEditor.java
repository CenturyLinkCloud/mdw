...
public class TaskTemplateEditor extends MultiPageEditorPart implements ElementChangeListener,
    ...
    protected void createPages() {
        ...
        try {
            ...
            PluginMessages.uiError(getSite().getShell(), ex, "Task Template",
            ...
            if (belongs) {
                if ("Notices".equals(propertyEditor.getName())) {
                    ...
                    if (StringHelper.isEmpty(attrVal) || attrVal.equals("$DefaultNotices"))
                        taskTemplate.setAttribute(TaskAttributeConstant.NOTICES,
                                TaskVO.getDefaultNotices());
                            ...
                            if (editable && getColumnIndex(property) == 1) {
                                for (VariableVO var : getProcessVariables()) {
                                    if (var.getName().equals(getValue(element, property)))
                                        return false; // can't edit process var
                                                      // rows
                                }
                            }
                            return editable;
                ...
                if (!propertyEditor.getType().equals(PropertyEditor.TYPE_LINK)) {
                    propertyEditor.setEditable(
                            !(propertyEditor.isReadOnly() || taskTemplate.isReadOnly()));
                }
            }
        }
    ...
    private int saveDialog(boolean closeButton) {
        VersionableSaveDialog saveDlg = new VersionableSaveDialog(getSite().getShell(),
                taskTemplate, closeButton);
        ...
        try {
            ...
            else // save the template
            {
                ...
                if (versionIncrement != Increment.Overwrite) {
                    taskTemplate.setVersion(
                            versionIncrement == Increment.Major ? taskTemplate.getNextMajorVersion()
                                    : taskTemplate.getNextMinorVersion());
                    taskTemplate.setRevisionComment(saveDlg.getVersionComment());
                    taskTemplate.getProject().getDesignerProxy()
                            .saveWorkflowAssetWithProgress(taskTemplate, false);
                    taskTemplate.fireElementChangeEvent(ChangeType.VERSION_CHANGE,
                            taskTemplate.getVersion());
                }
                dirtyStateChanged(false);
                if (!taskTemplate.getProject().isRemote())
                    taskTemplate.getProject().getDesignerProxy().getCacheRefresh()
                            .fireRefresh(false);
                return result;
            }
        }
        catch (Exception ex) {
            PluginMessages.uiError(getSite().getShell(), ex, "Save Task Template",
                    taskTemplate.getProject());
    ...
    class NoticesValueChangeListener extends AttributeValueChangeListener {
        ...
        public void attributeValueChanged(String newValue) {
            ...
            int columnCount = StringHelper
                    .delimiterColumnCount(newValue.substring(0, newValue.indexOf(";")), ",", "\\,");
