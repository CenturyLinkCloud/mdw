...
public class ViewLayoutTableTreeViewer extends XMLTableTreeViewer {
                ...
                if (object instanceof Node) {
                    ...
                    if (node.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        if (("list".equals(element.getNodeName())
                                || "filter".equals(element.getNodeName())
                                || "detail".equals(element.getNodeName())
                                || "chart".equals(element.getNodeName()))
                                && (element.getAttribute("id") != null
                                        && element.getAttribute("id").trim().length() > 0)) {
                            return element.getAttribute("id");
                        }
                        else if (("column".equals(element.getNodeName())
                                || "field".equals(element.getNodeName())
                                || "row".equals(element.getNodeName()))
    ...
    class ViewLayoutNodeActionMenuListener implements IMenuListener {
        public void menuAboutToShow(IMenuManager menuManager) {
            ...
            XMLNodeActionManager nodeActionManager = new XMLNodeActionManager(idom.getModel(),
                    ViewLayoutTableTreeViewer.this) {
                ...
                public void contributeActions(IMenuManager menu, List selection) {
                    ...
                    if (selection.size() == 1 && selection.get(0) instanceof IDOMElement) {
                        ...
                        IMenuManager addAttributeMenu = new MyMenuManager(
                                XMLUIMessages._UI_MENU_ADD_ATTRIBUTE);
                        IMenuManager addChildMenu = new MyMenuManager(
                                XMLUIMessages._UI_MENU_ADD_CHILD);

                        if ("view".equals(idomElem.getNodeName())) {
                            menu.add(addChildMenu);
                            List modelQueryActionList = new ArrayList();
                            modelQuery.getInsertActions(idomElem, ed, -1, ic, vc,
                            ...
                            contributeDeleteActions(menu, getSelectedNodes(selection, true), ic,
                                    vc);
                            ...
                            if (ed != null) {
                                // attribute actions
                                List modelQueryActionList = new ArrayList();
                                modelQuery.getInsertActions(idomElem, ed, -1,
                                        ModelQuery.INCLUDE_ATTRIBUTES, vc, modelQueryActionList);
                        ...
                        Action editAttributeAction = new EditAttributeAction(this, ownerElem,
                                idomAttr, XMLUIMessages._UI_MENU_EDIT_ATTRIBUTE,
                                XMLUIMessages._UI_MENU_EDIT_ATTRIBUTE_TITLE) {
                            public void run() {
                                Shell shell = MdwPlugin.getShell();
                                if (validateEdit(manager.getModel(), shell)) {
                                    manager.beginNodeAction(this);
                                    EditAttributeDialog dialog = new ViewLayoutEditAttributeDialog(
                                            shell, viewLayout, ownerElement, attr);
                                    dialog.create();
                                    ...
                                    if (dialog.getReturnCode() == Window.OK) {
                                        ...
                                        Attr newAttribute = document
                                                .createAttribute(dialog.getAttributeName());
