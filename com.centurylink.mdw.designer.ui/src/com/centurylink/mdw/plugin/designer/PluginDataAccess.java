...
/**
 * Provides access to the cached data collections stored on
 * Designer classic's MainFrame object.
 */
public class PluginDataAccess
{
  private WorkflowProject workflowProject;
  private DesignerDataModel designerDataModel;
  public DesignerDataModel getDesignerDataModel() { return designerDataModel; }
  private DesignerDataAccess designerDataAccess;
  DesignerDataAccess getDesignerDataAccess() { return designerDataAccess; }
  void setDesignerDataAccess(DesignerDataAccess designerDataAccess) { this.designerDataAccess = designerDataAccess; }

  public PluginDataAccess(WorkflowProject workflowProject, DesignerDataModel dataModel, DesignerDataAccess dataAccess)
  {
  ...
  public List<PackageVO> getPackages(boolean reload, ProgressMonitor progressMonitor)
  {
    if (packageVOs == null || reload)
    {
      try
      {
        ...
        packageVOs.add(0, determineDefaultPackage(packageVOs, designerDataModel.getProcesses()));
      }
      catch (final Exception ex)
      {
        // otherwise the message won't display because we're not on the UI thread
  ...
  public ProcessVO getProcess(String processName, String version)
  {
    ...
    {
      if (processVO.getProcessName().equals(processName) && processVO.getVersionString().equals(version))
        return processVO;
    }
    return null; // not found
  }
  ...
  public ProcessVO loadProcess(WorkflowProcess processVersion)
  {
    try
    {
      return designerDataAccess.getProcess(processVersion.getId(), processVersion.getProcessVO());
    }
    catch (Exception ex)
    {
  ...
  public void organizeRuleSets()
  {
    ...
    // use these since they've already been retrieved, even though we maintain a separate cache
  ...
  public void reloadUserPermissions()
  {
    try
    {
      designerDataModel.reloadPriviledges(getDesignerDataAccess(), workflowProject.getUser().getUsername());
    }
    catch (Exception ex)
    {
  ...
  {
    Map<String,List<String>> ownerVars = new TreeMap<String,List<String>>(PluginUtil.getStringComparator());
