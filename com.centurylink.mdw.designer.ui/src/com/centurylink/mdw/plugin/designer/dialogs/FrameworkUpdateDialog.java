...
public class FrameworkUpdateDialog extends TrayDialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        if (project.isRemote() || project.isOsgi())
            msgLabel.setText("The project '" + project.getSourceProjectName()
                    + "' will be updated with Maven dependencies during this required one-time step.");
        else
            msgLabel.setText(
                    "The MDW Framework Jar files will be downloaded locally\nand added to your classpath during this required one-time step.");
