...
public class VariableValueDialog extends TrayDialog
{
  ...
  public VariableValueDialog(Shell shell, VariableValue variableValue)
  {
    ...
    if (variableValue.getType() != null && variableValue.getType().isJavaObjectType())
      variableValue.setReadOnly(true);  // only updateable from var inst section
  }

  public VariableValueDialog(Shell shell, VariableInstanceInfo variableInstanceInfo, VariableTypeVO type, String value, boolean readOnly)
  {
    super(shell);
  ...
  {
    String type = variableValue.getType() == null ? null : variableValue.getType().getVariableType();
    if (isCollectionType(type))
  ...
  class CollectionModelUpdater extends TableEditor.DefaultModelUpdater
  {
    ...
    public void updateModelValue(List tableValue)
    {
      ...
      {
        String firstColStringVal = (String)((DefaultRowImpl)tableValue.get(i)).getColumnValue(0);

        if (firstColStringVal == null || firstColStringVal.trim().length() == 0)
          continue;
        if (!(validateValue(firstColStringVal, variableValue.getType().getVariableType()) == null))
          return;
        serialized += firstColStringVal;

        if (variableValue.getType().getVariableType().equals("java.util.Map"))
        {
          String secondColStringVal = (String)((DefaultRowImpl)tableValue.get(i)).getColumnValue(1);
          if (!(validateValue(secondColStringVal, variableValue.getType().getVariableType()) == null))
            return;
