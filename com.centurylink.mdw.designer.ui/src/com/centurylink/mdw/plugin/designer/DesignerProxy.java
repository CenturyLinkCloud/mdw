...
/**
 * Provides access to Designer functionality for the plug-in. This is useful
 * when dealing with UI wrapper objects or when long-running operations require
 * a progress monitor provided by the DesignerRunner class.
 */
public class DesignerProxy {
    private PluginDataAccess dataAccess;

    public PluginDataAccess getPluginDataAccess() {
        return dataAccess;
    }
    ...
    public AwtEnvironment getAwtEnvironment() {
    ...
    public void initialize(ProgressMonitor progressMonitor) throws Exception {
        ...
        try {
            ...
            if (project.getPersistType() == WorkflowProject.PersistType.Git) {
                ...
                int schemaVersion = project.getMdwMajorVersion() * 1000
                        + project.getMdwMinorVersion() * 100;
                restfulServer = new RestfulServer(jdbcUrl, project.getUser().getUsername(),
                        project.getServiceUrl(), schemaVersion);
                    ...
                    try {
                        if (isGit) {
                            ...
                            Platform.getBundle("org.eclipse.egit.ui").start(); // avoid
                                                                               ...
                                                                               // login
                                                                               // fails
                            ApplicationSummaryDocument appSummaryDoc = restfulServer
                                    .getAppSummary();
                            ApplicationSummary appSummary = appSummaryDoc.getApplicationSummary();
                            if (appSummary.getRepository() == null)
                                throw new DataAccessOfflineException(
                                        "Unable to confirm Git status on server (missing repository)");
                            String branch = appSummary.getRepository().getBranch();
                            if (branch == null || branch.isEmpty())
                                throw new DataAccessOfflineException(
                                        "Unable to confirm Git status on server (missing branch)");
                            ...
                            if (localCommit == null || !localCommit.equals(serverCommit)) {
                                ...
                                PluginMessages.log(
                                        "Server commit: " + serverCommit
                                                + " does not match Git repository for branch "
                                                + branch + ": " + versionControl.getCommit() + ".",
                                        IStatus.WARNING);
                            }
                            WorkflowProjectManager.getInstance().save(project); // save
                                                                                // the
                                                                                // discovered
                                                                                // branch
                            if (progressMonitor != null)
                            ...
                            if (!assetDir.mkdirs())
                                throw new DiscoveryException(
                                        "Unable to create asset directory: " + assetDir);
                        ...
                        if (pkgDownloadServicePath != null && progressMonitor != null) {
                            ...
                            String json = restfulServer
                                    .invokeResourceService(pkgDownloadServicePath);
                            ...
                            if (!StringHelper.isEmpty(download.getUrl())) {
                                ...
                                IProgressMonitor subMonitor = new SubProgressMonitor(
                                        ((SwtProgressMonitor) progressMonitor).getWrappedMonitor(),
                                        5);
                                try {
                                    PluginUtil.downloadIntoProject(project.getSourceProject(), url,
                                            tempFolder, tempFile, "Download Packages", subMonitor);
                                    PluginUtil.unzipProjectResource(project.getSourceProject(),
                                            tempFile, null, project.getAssetFolder(), subMonitor);
                                }
                                catch (FileNotFoundException ex) {
                                    if (isGit)
                                        throw new DataUnavailableException(
                                                "Extra/Archived packages not retrieved: "
                                                        + ex.getMessage(),
                                                ex);
                                    else
                                        throw ex;
                        ...
                        throw new DataUnavailableException(
                                "Extra/Archived packages not retrieved: " + ex.getMessage(), ex);
                    }
                }
            }
            else if (project.getPersistType() == WorkflowProject.PersistType.None) {
                restfulServer = new RestfulServer(null, project.getUser().getUsername(),
                ...
                if (jdbcUrl == null)
                    throw new DataAccessException(
                            "Please specify a valid JDBC URL in your MDW Project Settings");
                ...
                restfulServer = new RestfulServer(jdbcUrl, project.getUser().getUsername(),
                        project.getServiceUrl());
                connProps.put("defaultRowPrefetch",
                        String.valueOf(MdwPlugin.getSettings().getJdbcFetchSize()));
            }
            cacheRefresh = new CacheRefresh(project, restfulServer);

            boolean oldNamespaces = project.isOldNamespaces();
            boolean remoteRetrieve = project.isFilePersist()
                    && project.checkRequiredVersion(5, 5, 19);
            ...
            mainFrame.startSession(project.getUser().getUsername(), restfulServer, progressMonitor,
                    connProps, oldNamespaces, remoteRetrieve);
     ...
     */
    public ProcessVO loadProcess(String name, int version)
        ...
        dataAccess.getDesignerDataModel().addProcessGraph(new Graph(loaded,
                dataAccess.getDesignerDataModel().getNodeMetaInfo(), getIconFactory()));
        return loaded;
    }

    public void registerExternalEventHandler(ExternalEvent externalEvent) {
        try {
            dataAccess.getDesignerDataAccess()
                    .createExternalEvent(externalEvent.getExternalEventVO());
            if (!externalEvent.isInDefaultPackage())
                dataAccess.getDesignerDataAccess().addExternalEventToPackage(
                        externalEvent.getExternalEventVO(),
                        externalEvent.getPackage().getPackageVO());
    ...
    public void saveExternalEvent(ExternalEvent externalEvent) {
        try {
            dataAccess.getDesignerDataAccess()
                    .updateExternalEvent(externalEvent.getExternalEventVO());
            cacheRefresh.refreshSingle("ExternalEventCache", true);
        }
        catch (Exception ex) {
            if (ex.getCause() != null && ex.getCause().getMessage() != null
                    && ex.getCause().getMessage().startsWith(("ORA-00001")))
                PluginMessages.uiMessage(
                        "Message Pattern already exists.  External Event cannot be saved.",
                        "Save External Event", PluginMessages.INFO_MESSAGE);
            else
                PluginMessages.uiError(ex, "Create External Event Handler", project);
        }
    }
    ...
    public void deleteExternalEvent(final ExternalEvent externalEvent) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws DataAccessException, RemoteException {
                if (externalEvent.isInDefaultPackage()) {
                    externalEvent.getProject().getDefaultPackage()
                            .removeExternalEvent(externalEvent);
                }
                else {
                    dataAccess.getDesignerDataAccess().removeExternalEventFromPackage(
                            externalEvent.getExternalEventVO(),
                            externalEvent.getPackage().getPackageVO());
                    externalEvent.getPackage().removeExternalEvent(externalEvent);
                }
                dataAccess.getDesignerDataAccess()
                        .removeExternalEvent(externalEvent.getExternalEventVO());
            }
        };
        designerRunner.run();
    }
    ...
    public void createActivityImpl(ActivityImpl activityImpl) {
        try {
            dataAccess.getDesignerDataAccess()
                    .createActivityImplementor(activityImpl.getActivityImplVO(), false, null);
            if (!activityImpl.isInDefaultPackage())
                dataAccess.getDesignerDataAccess().addActivityImplToPackage(
                        activityImpl.getActivityImplVO(), activityImpl.getPackage().getPackageVO());
    ...
    public void saveActivityImpl(ActivityImpl activityImpl) {
        try {
            dataAccess.getDesignerDataAccess()
                    .updateActivityImplementor(activityImpl.getActivityImplVO(), false, null);
            cacheRefresh.fireRefresh(false);
        }
        catch (Exception ex) {
            if (ex.getCause() != null && ex.getCause().getMessage() != null
                    && ex.getCause().getMessage().startsWith(("ORA-00001")))
                PluginMessages.uiMessage("ActivityImplementor already exists and cannot be saved.",
                        "Save Activity Implementor", PluginMessages.INFO_MESSAGE);
            else
            ...
            public void perform() throws DataAccessException, RemoteException {
                for (ActivityImpl activityImpl : activityImpls) {
                    if (activityImpl.getActivityImplVO().isShowInToolbox())
                        dataAccess.getDesignerDataAccess().updateActivityImplementor(
                                activityImpl.getActivityImplVO(), false, null);
                }
                cacheRefresh.fireRefresh(false);
            }
        };
    ...
    public void deleteActivityImpl(final ActivityImpl activityImpl,
            final boolean includeActivities) {
            ...
            public void perform() throws DataAccessException, RemoteException, ValidationException {
                for (PackageVO packageVO : dataAccess.getPackages(false)) {
                    if (packageVO.containsActivityImpl(activityImpl.getId())) {
                        dataAccess.getDesignerDataAccess().removeActivityImplFromPackage(
                                activityImpl.getActivityImplVO(), packageVO);
                        project.getPackage(packageVO.getName()).removeActivityImpl(activityImpl);
                    }
                }
                try {
                    dataAccess.getDesignerDataAccess().removeActivityImplementor(
                            activityImpl.getActivityImplVO(), includeActivities);
                    dataAccess.getDesignerDataModel()
                            .removeImplementor(activityImpl.getActivityImplVO());
                }
                catch (DataAccessException ex) {
                    if (ex.getCause() != null && ex.getCause().getMessage() != null
                            && ex.getCause().getMessage().startsWith("ORA-02292")) {
                        PluginMessages.log(ex);
                        throw new ValidationException("It appears that activity implementor '"
                                + activityImpl.getName()
                                + "' is used by one or more processes.  Please delete the processes before proceeding.");
                    }
                    else {
                        throw ex;
                    }
    ...
    public ActivityImpl getGenericActivityImpl(String implClass) {
        ActivityImplementorVO implVO = dataAccess.getDesignerDataModel().getNodeMetaInfo()
                .getDefaultActivity();
        if (implVO == null) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                if (!asset.getProject().isFilePersist()) {
                    // check for existing
                    RuleSetVO existing = dataAccess.getDesignerDataAccess().getRuleSet(
                            asset.getPackage() == null ? 0 : asset.getPackage().getId(),
                            asset.getName());
                    if (existing != null && existing.getVersion() == asset.getVersion())
                        throw new ValidationException(asset.getFullPathLabel() + ALREADY_EXISTS
                                ...
                                + "     - Recover changes from backup under "
                                + asset.getTempFolder().getFullPath() + "\n"
    ...
    public void saveWorkflowAsset(WorkflowAsset asset, boolean keepLocked)
        ...
        asset.setModifyDate(new Date()); // roughly the same as db time
                                         // hopefully

        WorkflowPackage pkg = asset.getPackage();
        if (pkg != null && !pkg.isDefaultPackage()) {
            if (project.isFilePersist()) {
                // for raw assets, saveWorkflowAsset() is only called when
                // version incremented, so pkg needs saving
                pkg.setVersion(pkg.getVersion() + 1);
                pkg.setExported(false); // exported not relevant for vcs
    ...
    public boolean saveOverrideAttributes(String prefix, String ownerType, Long ownerId,
            String subType, String subId, Map<String, String> attributes) {
        try {
            try {
                dataAccess.getDesignerDataAccess().setOverrideAttributes(prefix, ownerType, ownerId,
                        subType, subId, attributes);
            }
            catch (DataAccessOfflineException ex) {
                PluginMessages.log(ex);
                MessageDialog.openWarning(MdwPlugin.getShell(), "Save Attributes",
            ...
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                if (dataAccess.getDesignerDataModel().getNodeMetaInfo().getStartActivity() == null)
                    throw new ValidationException(
                            "Please import the MDW Baseline Package corresponding to your MDW framework version.");

                Graph process = new Graph(processVersion.getName(),
                        ProcessVisibilityConstant.PUBLIC,
                        dataAccess.getDesignerDataModel().getNodeMetaInfo(), getIconFactory());
                process.setDescription(processVersion.getDescription());
                process.getProcessVO().setInRuleSet(processVersion.isInRuleSet());
                if (project.isFilePersist())
                    process.getProcessVO()
                            .setRawFile(new File(project.getAssetDir() + "/"
                                    + processVersion.getPackage().getName() + "/"
                                    + processVersion.getName() + ".proc"));
    ...
    public void saveProcessAs(final WorkflowProcess processVersion,
            final WorkflowPackage targetPackage, final String newName, final boolean validate) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                ProcessVO newProcVO = new ProcessVO(-1L, newName,
                        origProcVO.getProcessDescription(), null);
                newProcVO.set(origProcVO.getAttributes(), origProcVO.getVariables(),
                        origProcVO.getTransitions(), origProcVO.getSubProcesses(),
                        origProcVO.getActivities());
                newProcVO.setVersion(1);
                newProcVO.setInRuleSet(origProcVO.isInRuleSet());
                WorkflowProcess newProcess = new WorkflowProcess(targetPackage.getProject(),
                        newProcVO);
                newProcess.setPackage(targetPackage);

                Graph process = new Graph(newProcVO,
                        dataAccess.getDesignerDataModel().getNodeMetaInfo(), getIconFactory());
                process.dirtyLevel = Graph.NEW;
                // mainFrame.procmenu.dataChanged(new
                // DataChangeEvent(DataChangeEvent.DEFINITION_GRAPH_ADD,
                // process, false));
    ...
    public void createNewPackage(final WorkflowPackage newPackage, final boolean isJson) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws DataAccessException, RemoteException, ValidationException {
                ...
                // update the tree
                newPackage.getProject().fireElementChangeEvent(ChangeType.ELEMENT_CREATE,
                        newPackage);
    ...
    public void renamePackage(final WorkflowPackage packageToRename, final String newName) {
        ...
        if (workflowProject.packageNameExists(newName)) {
            ...
            MessageDialog.openError(shell, "Can't Rename",
                    "Package name already exists: '" + newName + "'");
            return;
        }
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, workflowProject) {
            public void perform() throws DataAccessException, RemoteException {
                packageToRename.getPackageVO().setId(dataAccess.getDesignerDataAccess()
                        .renamePackage(packageToRename.getId(), newName, 1));
            }
        };
        designerRunner.run();
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, asset.getProject()) {
            public void perform() throws DataAccessException, RemoteException {
                ...
                if (asset instanceof AutomatedTestCase) {
                    if (project.isFilePersist())
                        ((AutomatedTestCase) asset).setTestCase(
                                new TestCase(asset.getPackage().getName(), asset.getRawFile()));
                    else
                        ((AutomatedTestCase) asset).setTestCase(
                                new TestCase(asset.getPackage().getName(), asset.getRuleSetVO()));
                }
                asset.setModifyDate(new Date()); // roughly the same as db time
                                                 // hopefully
                cacheRefresh.fireRefresh(RuleSetVO.JAVA.equals(asset.getLanguage()));
    ...
    public List<WorkflowProcess> getProcessesUsingActivityImpl(Long activityImplId,
            String implClass) throws DataAccessException {
        List<WorkflowProcess> processVersions = new ArrayList<WorkflowProcess>();
        for (ProcessVO processVO : dataAccess.getDesignerDataAccess()
                .getProcessListForImplementor(activityImplId, implClass)) {
    ...
    public void renameProcess(final WorkflowProcess processVersion, final String newName) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform()
                ...
                try {
                    if (dataAccess.getDesignerDataAccess()
                            .hasProcessInstances(processVersion.getId()))
                    ...
                    MdwPlugin.getDisplay().syncExec(new Runnable() {
                        public void run() {
                            ...
                            confirm.append(MessageDialog.openConfirm(MdwPlugin.getShell(),
                                    "Rename Process", msg));
                        }
                    });
                ...
                if (processVersion.isInRuleSet() && !project.isFilePersist()) {
                    ProcessVO procVO = dataAccess.getDesignerDataAccess().getProcessDefinition(
                            processVersion.getName(), processVersion.getVersion());
                    procVO = dataAccess.getDesignerDataAccess().getProcess(procVO.getProcessId(),
                            procVO);
                    ...
                    processVersion.getProcessVO().setId(dataAccess.getDesignerDataAccess()
                            .renameProcess(processVersion.getId(), newName, 1));
                }
                dataAccess.getDesignerDataModel().addProcess(processVersion.getProcessVO());
            }
        };
    ...
    public void deleteProcess(final WorkflowProcess processVersion,
            final boolean includeInstances) {
        String version = "v" + processVersion.getVersionString();
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws DataAccessException, RemoteException, ValidationException {
                ...
                dataAccess.getDesignerDataAccess().removeProcess(processVersion.getProcessVO(),
                        includeInstances);
                dataAccess.removeProcess(processVersion.getProcessVO());
                if (processVersion.isInDefaultPackage()) {
    ...
    public void savePackage(WorkflowPackage packageVersion)
            throws DataAccessException, RemoteException, ValidationException {
        savePackage(packageVersion, false);
    }

    public void savePackage(WorkflowPackage packageVersion, boolean newVersion)
            throws DataAccessException, RemoteException, ValidationException {
        ...
        if (packageVersion.getPackageVO().getImplementors() != null) {
            ...
            for (ActivityImplementorVO activityImpl : packageVersion.getPackageVO()
                    .getImplementors()) {
                if (dataAccess
                        .getActivityImplementor(activityImpl.getImplementorClassName()) == null)
        ...
        if (packageVersion.getPackageVO() != null
                && packageVersion.getPackageVO().getAttributes() != null) {
        ...
        if (!packageVersion.isLatest() || packageVersion.isArchived()) // do not
                                                                       // create
                                                                       // new
                                                                       // version
            ...
            pkgId = dataAccess.getDesignerDataAccess().savePackage(packageVersion.getPackageVO());
        if (prevPkg.getId() == null || newVersion
                || (pkgId.longValue() != prevPkg.getId().longValue())) {
            ...
            // update the tree
            packageVersion.getProject().fireElementChangeEvent(packageVersion,
                    ChangeType.VERSION_CHANGE, packageVersion.getVersionString());
            // update other listeners
            packageVersion.fireElementChangeEvent(ChangeType.VERSION_CHANGE,
    ...
    public void setPackage(WorkflowPackage workflowPackage)
        ...
        workflowPackage.getProject().fireElementChangeEvent(workflowPackage,
                ChangeType.VERSION_CHANGE, workflowPackage.getVersionString());
        workflowPackage.fireElementChangeEvent(ChangeType.VERSION_CHANGE,
                workflowPackage.getVersionString());
        timer.stopAndLog();
    }

    public void tagPackage(WorkflowPackage packageVersion, String tag)
            throws DataAccessException, RemoteException, ValidationException {
        if (!packageVersion.isExported()) {
            ...
            packageVersion.fireElementChangeEvent(ChangeType.STATUS_CHANGE,
                    WorkflowPackage.STATUS_EXPORTED);
        }
        String existingTags = packageVersion.getAttribute(WorkAttributeConstant.VERSION_TAG);
        String newTags = (existingTags == null || existingTags.isEmpty()) ? tag
                : existingTags + "," + tag;
        packageVersion.setAttribute(WorkAttributeConstant.VERSION_TAG, newTags);
        dataAccess.setAttribute(OwnerType.PACKAGE, packageVersion.getId(),
                WorkAttributeConstant.VERSION_TAG, newTags);
        packageVersion.fireElementChangeEvent(ChangeType.SETTINGS_CHANGE,
                packageVersion.getAttributeVO(WorkAttributeConstant.VERSION_TAG));
    }

    public void copyForeignProcess(final String name, final String version,
            final WorkflowProject sourceProject, final WorkflowPackage targetPackage) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform()
                ...
                Exporter exporter = new Exporter(
                        sourceProject.getDesignerProxy().getDesignerDataAccess());
    ...
    public String exportProcess(WorkflowProcess processVersion)
            throws DataAccessException, RemoteException, XmlException {
        return getDesignerDataAccess().exportProcess(processVersion.getId(),
                processVersion.getProject().isOldNamespaces());
    }

    public void saveProcessWithProgress(final WorkflowProcess process,
            final FlowchartPage flowchartPage, final PersistType persistType, final int version,
            final boolean validate, final boolean keepLocked) {
            ...
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (persistType == PersistType.NEW_VERSION) {
                    ...
                    if (process.isInRuleSet())
                        existing = dataAccess.getDesignerDataAccess().getRuleSet(process.getName(),
                                RuleSetVO.PROCESS, version);
                    if (existing != null)
                    ...
                    if (project.isFilePersist()) // temp attribute to avoid API
                                                 // change
                        process.setAttribute("previousProcessVersion",
                                oldProcess.getVersionString());
                }
    ...
    public RunnerResult forceUpdateProcessWithProgress(final WorkflowProcess processVersion,
            final FlowchartPage flowchartPage, final boolean validate, final boolean keepLocked) {
     ...
     * data.
     */
    public void saveProcess(WorkflowProcess process, FlowchartPage flowchartPage,
            PersistType persistType, int version, boolean validate, boolean lock)
            throws ValidationException, DataAccessException, RemoteException {
        ...
        // static cache
        dataAccess.loadVariableTypes(); // calls
                                        // VariableTypeCache.refreshCache()

        if (validate)
            new ProcessValidator(process.getProcessVO()).validate(getNodeMetaInfo());

        if (!process.getProject().checkRequiredVersion(5, 5))
            flowchartPage.setProcessVersions(flowchartPage.getProcess(),
                    !process.getProject().checkRequiredVersion(5, 2)); // not
                                                                       // sure
                                                                       // why
                                                                       // this
                                                                       // was
        ...
        if (process.getProcessVO() != null && process.getProject().isFilePersist()
        ...
        try {
            ...
            if (ex.getMessage() != null && ex.getMessage().contains("ORA-02292"))
                throw new ValidationException(
                        "There are instances associated with this process version.\n       Please increment the version when saving.");
            else
                throw ex;
        }
        ...
        if (process.getProject().isFilePersist() && version != 0
                && persistType == PersistType.NEW_VERSION && overrideAttributes != null
                && !overrideAttributes.isEmpty() && MdwPlugin.getDefault().getPreferenceStore()
                        .getBoolean(PreferenceConstants.PREFS_CARRY_FORWARD_OVERRIDE_ATTRS)) {
     ...
     * data.
     */
    public void forceUpdateProcess(WorkflowProcess processVersion, FlowchartPage flowchartPage,
            boolean validate, boolean lock)
            throws ValidationException, DataAccessException, RemoteException {
        ...
        if (!processVersion.getProject().checkRequiredVersion(5, 5))
            flowchartPage.setProcessVersions(flowchartPage.getProcess(),
                    !processVersion.getProject().checkRequiredVersion(5, 2)); // not
                                                                              // sure
                                                                              // why
                                                                              // this
                                                                              // was
    ...
    public boolean hasInstances(WorkflowProcess processVersion)
    ...
    public ProcessList getProcessInstanceList(WorkflowProcess processVersion,
            Map<String, String> criteria, Map<String, String> variables, int pageIndex,
        ...
        try {
            ...
            if (id == 0) {
                ...
                String name;
                if (project.isFilePersist())
                    name = processVersion.getPackage().getName() + "/" + processVersion.getName(); // qualified
                                                                                                   // name
                else
                    name = processVersion.getName();

                return dataAccess.getDesignerDataAccess().getProcessInstanceList(name, criteria,
                        variables, pageIndex, pageSize, orderBy);
            }
            else {
                return dataAccess.getDesignerDataAccess().getProcessInstanceList(id, criteria,
                        variables, pageIndex, pageSize, orderBy);
            }
        }
        catch (DataAccessOfflineException ex) {
            ...
            return new ProcessList(ProcessList.PROCESS_INSTANCES,
                    new ArrayList<ProcessInstanceVO>());
        }
        catch (Exception ex) {
    ...
    public List<ProcessInstanceVO> getProcessInstances(Map<String, String> criteria)
            throws DataAccessException, RemoteException {
        CodeTimer timer = new CodeTimer("getProcessInstances()");
        try {
            return dataAccess.getDesignerDataAccess().getProcessInstanceList(criteria, 1, 50,
                    new ProcessVO(), "order by process_instance_id desc").getItems();
        }
        finally {
            timer.stopAndLog();
        }
    ...
    public RunTimeDesignerCanvas loadProcessInstance(WorkflowProcess processVersion,
        ...
        ProcessInstanceLoader instanceLoadThread = new ProcessInstanceLoader(processInstanceInfo,
                processInstancePage);

        String errorMessage = null;

        try {
            Graph processGraph = dataAccess.getDesignerDataModel()
                    .findProcessGraph(processVersion.getId(), null);
            if (processGraph == null)
                processGraph = processInstancePage.loadProcess(processInstanceInfo.getProcessId(),
                        null);
            processVersion.setProcessVO(processGraph.getProcessVO());

            DesignerDataAccess dao = processVersion.getDesignerDataAccess() == null
                    ? processInstancePage.frame.dao : processVersion.getDesignerDataAccess();
            Graph procInstGraph = instanceLoadThread.loadCompletionMap(processGraph.getProcessVO(),
                    processInstanceInfo, dao);
                        ...
                        if (instSubGraph.nodes != null) {
                            ...
                            for (Node node : instSubGraph.nodes) {
                                if (node.isTaskActivity()) {
                                    for (ProcessInstanceVO embeddedProcInst : instSubGraph
                                            .getInstances()) {
                                        if (!embeddedProcInst
                                                .getActivityInstances(node.getActivityId())
                                                .isEmpty())
                            ...
                            if (!taskActivityIds.isEmpty()) {
                                for (ProcessInstanceVO embeddedProcInst : instSubGraph
                                    ...
                                    Map<Long, List<TaskInstanceVO>> embeddedTaskInsts = dao
                                            .getTaskInstances(embeddedProc, embeddedProcInst,
                                                    taskActivityIds);
                                    for (Long actId : embeddedTaskInsts.keySet()) {
                                        if (taskInstances.get(actId) == null)
                                            taskInstances.put(actId,
                                                    new ArrayList<TaskInstanceVO>());
                                        taskInstances.get(actId)
                                                .addAll(embeddedTaskInsts.get(actId));
                                    }
                                }
                            }
                        }
            ...
            if (processGraph.nodes != null) {
                ...
                for (Node node : processGraph.nodes) {
                    if (node.isTaskActivity() && !processInstanceInfo
                            .getActivityInstances(node.getActivityId()).isEmpty())
                        taskActivityIds.add(node.getActivityId());
                }
                if (!taskActivityIds.isEmpty()) {
                    Map<Long, List<TaskInstanceVO>> taskInsts = dao.getTaskInstances(
                            processVersion.getProcessVO(), processInstanceInfo, taskActivityIds);
    ...
    public WorkflowAsset loadWorkflowAsset(WorkflowAsset asset) {
        try {
            ...
            if (!(asset instanceof TaskTemplate)) // don't overwrite task
                                                  // attributes
    ...
    public List<ProcessInstanceVO> getSubProcessInstances(WorkflowProcess parentProcess,
            Activity activity) {
        try {
            ...
            if (activity.isHeterogeneousSubProcInvoke()) {
                ...
                if (procMapStr != null && !procMapStr.isEmpty()) {
                    ...
                    for (String[] row : procMap) {
                        ...
                        AssetVersionSpec spec = new AssetVersionSpec(row[1],
                                row[2] == null ? "0" : row[2]);
                        AssetLocator locator = new AssetLocator(activity,
                        ...
                        if (found != null) {
                            ...
                            insts.addAll(dataAccess.getDesignerDataAccess().getChildProcessInstance(
                                    parentProcessInstanceId, subProcessVO,
                                    parentProcess.getProcessVO()));
                        }
                        else {
                            PluginMessages.log(new Exception(
                                    "SubProcess not found: " + row[1] + " v" + row[2]));
                        }
                    }
                }
                return insts;
                ...
                if (procMapStr != null && !procMapStr.isEmpty()) {
                    ...
                    insts = dataAccess
                            .getDesignerDataAccess().getProcessInstanceList(pMap, 0,
                                    QueryRequest.ALL_ROWS, parentProcess.getProcessVO(), null)
                            .getItems();
                }
                return insts;
            }
                ...
                AssetVersionSpec spec = new AssetVersionSpec(subProcName,
                        subProcVer == null ? "0" : subProcVer);
                ...
                subProcessVO.setProcessId(
                        (subProc == null || subProc.getId() == null) ? 0L : subProc.getId());
                subProcessVO
                        .setProcessName(activity.getAttribute(WorkAttributeConstant.PROCESS_NAME));

                // handle alias subprocs
                String subprocAliasProcessId = activity
                        .getAttribute(WorkAttributeConstant.ALIAS_PROCESS_ID);
                if (subprocAliasProcessId != null)
                    subProcessVO = this.getProcessVO(new Long(subprocAliasProcessId));

                return dataAccess.getDesignerDataAccess().getChildProcessInstance(
                        parentProcessInstanceId, subProcessVO, parentProcess.getProcessVO());
            }
        }
        catch (Exception ex) {
            PluginMessages.uiError(ex, "Load SubProcess Instances (P=" + parentProcess.getId()
                    + ",A=" + activity.getId() + ")", project);
            return null;
        }
    }

    public List<ProcessInstanceVO> getSubProcessInstances(
            ProcessInstanceVO parentEmbeddedProcessInstance, Activity activity) {
        try {
            Long parentProcessInstanceId = parentEmbeddedProcessInstance.getId();
            ProcessVO subProcessVO = new ProcessVO();
            String subProcName = activity.getAttribute(WorkAttributeConstant.PROCESS_NAME);
            String subProcVer = activity.getAttribute(WorkAttributeConstant.PROCESS_VERSION);
            WorkflowProcess subProc = new AssetLocator(activity, AssetLocator.Type.Process)
                    .getProcessVersion(new AssetVersionSpec(subProcName, subProcVer));
            subProcessVO.setProcessId(subProc == null ? 0L : subProc.getId());
            subProcessVO.setProcessName(subProcName);

            return dataAccess.getDesignerDataAccess().getChildProcessInstance(
                    parentProcessInstanceId, subProcessVO, new ProcessVO());
        }
        catch (Exception ex) {
    ...
    public void moveProcessToPackage(final String processName, final String version,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Moving '" + processName + " v " + version + "' to package '"
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (processVersion.isInDefaultPackage()) {
                    if (processVersion.isInRuleSet())
                        dataAccess.getDesignerDataAccess().addRuleSetToPackage(
                                processVersion.getProcessVO(), targetPackage.getPackageVO());
                    else
                        dataAccess.getDesignerDataAccess().addProcessToPackage(
                                processVersion.getProcessVO(), targetPackage.getPackageVO());
                    WorkflowPackage defaultPackage = workflowProject.getDefaultPackage();
                    ...
                    if (processVersion.isInRuleSet()) {
                        ...
                        Long newId = dataAccess.getDesignerDataAccess().addRuleSetToPackage(
                                processVersion.getProcessVO(), targetPackage.getPackageVO());
                        dataAccess.getDesignerDataAccess().removeRuleSetFromPackage(
                                processVersion.getProcessVO(),
                                processVersion.getPackage().getPackageVO());
                        ...
                        dataAccess.getDesignerDataAccess().addProcessToPackage(
                                processVersion.getProcessVO(), targetPackage.getPackageVO());
                        dataAccess.getDesignerDataAccess().removeProcessFromPackage(
                                processVersion.getProcessVO(),
                                processVersion.getPackage().getPackageVO());
                        processVersion.getPackage().removeProcess(processVersion);
                    }
    ...
    public void removeProcessFromPackage(final String processName, final String version,
            final WorkflowPackage packageVersion) {
        String progressMsg = "Removing '" + processName + " v " + version + "' from package '"
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (processVersion.isInRuleSet())
                    ...
                    dataAccess.getDesignerDataAccess().removeProcessFromPackage(
                            processVersion.getProcessVO(), packageVersion.getPackageVO());
    ...
    public void copyProcess(final String originalName, final String originalVersion,
            final String newName, final WorkflowPackage targetPackage) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                ProcessVO origProcVO = dataAccess.getDesignerDataAccess()
                        .getProcessDefinition(originalName, major * 1000 + minor);
                origProcVO = dataAccess.getDesignerDataAccess()
                        .getProcess(origProcVO.getProcessId(), origProcVO);
    ...
    public void moveExternalEventToPackage(final Long externalEventId,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Moving external event id '" + externalEventId + "' to package '"
                + targetPackage.getName() + "'";
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (externalEvent.isInDefaultPackage()) {
                    dataAccess.getDesignerDataAccess().addExternalEventToPackage(
                            externalEvent.getExternalEventVO(), targetPackage.getPackageVO());
                    WorkflowPackage defaultPackage = workflowProject.getDefaultPackage();
                    defaultPackage.removeExternalEvent(externalEvent);
                }
                else {
                    // id can change from repackaging for VCS assets
                    Long newId = dataAccess.getDesignerDataAccess().addExternalEventToPackage(
                            externalEvent.getExternalEventVO(), targetPackage.getPackageVO());
                    dataAccess.getDesignerDataAccess().removeExternalEventFromPackage(
                            externalEvent.getExternalEventVO(),
                            externalEvent.getPackage().getPackageVO());
    ...
    public void removeExternalEventFromPackage(final Long externalEventId,
            final WorkflowPackage packageVersion) {
        String progressMsg = "Removing external event id '" + externalEventId + "' from package '"
                + packageVersion.getName() + "'";
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                dataAccess.getDesignerDataAccess().removeExternalEventFromPackage(
                        externalEvent.getExternalEventVO(), packageVersion.getPackageVO());
    ...
    public void addActivityImplToPackage(final Long activityImplId,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Adding activity implementor id '" + activityImplId + "' to package '"
                + targetPackage.getName() + "'";
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                dataAccess.getDesignerDataAccess().addActivityImplToPackage(
                        activityImpl.getActivityImplVO(), targetPackage.getPackageVO());
    ...
    public void removeActivityImplFromPackage(final Long activityImplId,
            final WorkflowPackage packageVersion) {
        String progressMsg = "Removing activity implementor id '" + activityImplId
                + "' from package '"
                + (packageVersion == null ? "(default package)" : packageVersion.getName()) + "'";
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (packageVersion == null) // default
                {
                    ...
                    dataAccess.getDesignerDataAccess().removeActivityImplFromPackage(
                            activityImpl.getActivityImplVO(), packageVersion.getPackageVO());
    ...
    public void moveWorkflowAssetToPackage(final Long assetId,
            final WorkflowPackage targetPackage) {
        String progressMsg = "Moving Asset ID: " + assetId + " to package '"
                + targetPackage.getName() + "'";
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (asset.isInDefaultPackage()) {
                    dataAccess.getDesignerDataAccess().addRuleSetToPackage(asset.getRuleSetVO(),
                            targetPackage.getPackageVO());
                    WorkflowPackage defaultPackage = workflowProject.getDefaultPackage();
                    ...
                    // id can change from repackaging for VCS assets
                    Long newId = dataAccess.getDesignerDataAccess().addRuleSetToPackage(
                            asset.getRuleSetVO(), targetPackage.getPackageVO());
                    dataAccess.getDesignerDataAccess().removeRuleSetFromPackage(
                            asset.getRuleSetVO(), asset.getPackage().getPackageVO());
                    asset.setId(newId);
                    asset.getPackage().removeAsset(asset);
                }
    ...
    public void removeWorkflowAssetFromPackage(final Long assetId,
            final WorkflowPackage packageVersion) {
        String progressMsg = "Removing Asset ID: " + assetId + " from package '"
                + packageVersion.getName() + "'";
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                dataAccess.getDesignerDataAccess().removeRuleSetFromPackage(asset.getRuleSetVO(),
                        packageVersion.getPackageVO());
    ...
    public void updateProcessesAndAssetsToLatest(final WorkflowPackage packageVersion) {
        String progressMsg = "Updating package '" + packageVersion.getLabel()
                + "' to contain latest processes and workflow assets.";
            ...
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                for (WorkflowProcess inProc : pkgProcesses) {
                    ...
                    if (!inProc.equals(latestProc)) {
                        if (inProc.isInRuleSet()) {
                            dataAccess.getDesignerDataAccess().removeRuleSetFromPackage(
                                    inProc.getProcessVO(), packageVersion.getPackageVO());
                            packageVersion.removeProcess(inProc);
                            dataAccess.getDesignerDataAccess().addRuleSetToPackage(
                                    latestProc.getProcessVO(), packageVersion.getPackageVO());
                ...
                for (WorkflowAsset inAsset : pkgAssets) {
                    ...
                    if (!inAsset.equals(latestAsset)) {
                        dataAccess.getDesignerDataAccess().removeRuleSetFromPackage(
                                inAsset.getRuleSetVO(), packageVersion.getPackageVO());
                        packageVersion.removeAsset(inAsset);
                        dataAccess.getDesignerDataAccess().addRuleSetToPackage(
                                latestAsset.getRuleSetVO(), packageVersion.getPackageVO());
     ...
     */
    public void upgradeAssets(WorkflowPackage packageVersion)
            throws DataAccessException, IOException, RemoteException {
        ...
        for (ProcessVO processVO : processVOs) {
            ...
            ProcessVO newProc = dataAccess.getDesignerDataAccess()
        ...
        if (packageUpdated) {
            ...
            packageVersion.setExported(false); // avoid forcing version
                                               // increment on save
    ...
    public List<WorkflowProcess> findCallingProcesses(WorkflowProcess subproc)
            throws DataAccessException, RemoteException {
        List<WorkflowProcess> callers = new ArrayList<WorkflowProcess>();
        List<ProcessVO> callingProcVos = dataAccess.getDesignerDataAccess()
                .findCallingProcesses(subproc.getProcessVO());
    ...
    public List<WorkflowProcess> findCalledProcesses(WorkflowProcess mainproc)
            throws DataAccessException, RemoteException {
        List<WorkflowProcess> called = new ArrayList<WorkflowProcess>();
        List<ProcessVO> calledProcVos = dataAccess.getDesignerDataAccess()
                .findCalledProcesses(mainproc.getProcessVO());
    ...
    public LinkedProcessInstance getProcessInstanceCallHierarchy(ProcessInstanceVO procInst)
            throws DataAccessException, RemoteException {
        return dataAccess.getDesignerDataAccess().getProcessInstanceCallHierarchy(procInst);
    }

    public void updateVariableValue(final WorkflowProcess processVersion,
            final VariableInstanceInfo varInstInfo, final String newValue) {
        ...
        designerRunner = new DesignerRunner(progressMsg, errorMsg, project) {
            public void perform() throws ValidationException, DataAccessException, RemoteException {
                ...
                if (project.isFilePersist() || vt == null || varInstInfo.getInstanceId() == null
                        || isJavaObj) {
                    // vcs assets or unknown translator or new instance or java
                    // object (go to server)
                    try {
                        DocumentReference docRef = varInstInfo.getStringValue() != null
                                && varInstInfo.getStringValue().startsWith("DOCUMENT:")
                                        ? new DocumentReference(varInstInfo.getStringValue())
                                        : null;
                        varInstInfo.setStringValue(
                                String.valueOf(processVersion.getProcessInstance().getId()));
                        dataAccess.getDesignerDataAccess()
                                .updateVariableInstanceThruServer(varInstInfo, newValue, isDoc);
                        if (docRef != null)
                            varInstInfo.setStringValue(docRef.toString()); // restore
                                                                           // doc
                                                                           // ref
                                                                           // value
                        else
                        ...
                        if (ex.getCause() instanceof java.net.ConnectException) {
                            PluginMessages.log(ex);
                            throw new DataAccessException(
                                    "Updating this variable requires that your server is running and accessible.");
                        }
                        else {
                            throw ex;
                        }
                    ...
                    dataAccess.getDesignerDataAccess().updateVariableInstanceInDb(varInstInfo,
                            newValue, isDoc);
                }
            }
        };
    ...
    public VariableTypeVO getVariableInfo(final VariableInstanceInfo varInstInfo)
        ...
        final VariableTypeVO varTypeVO = new VariableTypeVO(0L, varType,
                varTrans == null ? null : varTrans.getClass().getName());
        if (varInstInfo.getStringValue() != null && varTypeVO.isJavaObjectType()) {
            BusyIndicator.showWhile(MdwPlugin.getShell().getDisplay(), new Runnable() {
                public void run() {
                    ...
                    try {
                        Object obj = new JavaObjectTranslator()
                                .realToObject(getDocument(docRef).getContent());
                        ...
                        if (MdwPlugin.getSettings().isLogConnectErrors())
                            PluginMessages.log(ex);
                        try {
                            String resp = getRestfulServer().invokeResourceService(
                                    "DocumentValue?format=xml&id=" + docRef.getDocumentId()
                                            + "&type=" + varTypeVO.getVariableType());
                            Resource res = Resource.Factory.parse(resp,
                            ...
                            for (Parameter param : res.getParameterList()) {
                                if ("className".equals(param.getName()))
                                    ...
                                    varTypeVO.setUpdateable(
                                            Boolean.parseBoolean(param.getStringValue()));
                            }
                        }
                        catch (Exception ex2) {
     ...
     * Must be called from the UI thread.
     */
    public String getVariableValue(final Shell shell, final VariableInstanceInfo varInstInfo,
            final boolean tryServer) throws DataAccessException, RemoteException, ConnectException {
                    ...
                    if (isDoc) {
                        ...
                        if (isJavaObj) {
                            try {
                                ...
                                if (tryServer) {
                                    try {
                                        value = getRestfulServer()
                                                .invokeResourceService("DocumentValue?id="
                                                        + docRef.getDocumentId() + "&type="
                                                        + varInstInfo.getType() + "&format=text");
                                    }
                                    catch (Exception ex2) {
                                        throw new RuntimeException(ex2.getMessage(), ex2);
                                    }
    ...
    private VariableTranslator getVariableTranslator(String varType) {
        ...
        if (vo != null) {
            try {
                Class<? extends VariableTranslator> cl = Class.forName(vo.getTranslatorClass())
                        .asSubclass(VariableTranslator.class);
    ...
    public ExternalMessageVO getExternalMessage(Activity activity,
        ...
        try {
            ...
            return dataAccess.getDesignerDataAccess().getExternalMessage(activity.getId(),
                    activityInstance.getId(), eventDocId);
        }
        catch (Exception ex) {
    ...
    public String retryActivityInstance(Activity activity, ActivityInstanceVO activityInstance) {
        try {
            String statusMsg = restfulServer.retryActivityInstance(activity.getId(),
                    activityInstance.getId(), activity.getProject().isOldNamespaces());
            getPluginDataAccess().auditLog(Action.Retry, Entity.ActivityInstance,
                    activityInstance.getId(), activity.getLabel());
            return statusMsg;
        }
        catch (Exception ex) {
            PluginMessages.log(ex);
    ...
    public String skipActivityInstance(Activity activity, ActivityInstanceVO activityInstance,
            String completionCode) {
        try {
            String statusMsg = restfulServer.skipActivityInstance(activity.getId(),
                    activityInstance.getId(), completionCode,
                    activity.getProject().isOldNamespaces());
            getPluginDataAccess().auditLog(Action.Proceed, Entity.ActivityInstance,
                    activityInstance.getId(), completionCode);
            return statusMsg;
        }
        catch (Exception ex) {
    ...
    public TestCaseRun prepareTestCase(AutomatedTestCase testCase, int runNum, File resultDir,
        ...
        try {
            ...
            else if (!testCase.isGroovy() && !testCase.isGherkin()
                    && masterRequestId.equals(TestDataFilter.AnyNumberToken))
                masterRequestId = Long.toString(System.currentTimeMillis());
            if (testCase.isGherkin()) {
                if (masterRequestId.indexOf("${masterRequestId}") != -1)
                    masterRequestId = masterRequestId.replace("${masterRequestId}",
                            project.getUser().getUsername() + "-" + sdf.format(new Date()));
            }
            testCase.setMasterRequestId(masterRequestId);
            TestCaseRun run;

            if (testCase.isGherkin()) {
                run = new GherkinTestCaseLaunch(testCase.getTestCase(), runNum, masterRequestId,
                ...
                run = new TestCaseRun(testCase.getTestCase(), runNum, masterRequestId,
                        new DesignerDataAccess(dataAccess.getDesignerDataAccess()), monitor,
                        procCache, testCase.isLoadTest(), true,
                        testCase.getProject().isOldNamespaces());
            }
    ...
    public MDWStatusMessage launchProcess(WorkflowProcess processVersion, String masterRequestId,
            String owner, Long ownerId, List<VariableValue> variableValues, Long activityId)
        ...
        MDWStatusMessageDocument statusMessageDoc = restfulServer.launchProcess(
                processVersion.getId(), masterRequestId, owner, ownerId, variables, activityId,
                project.isOldNamespaces());
        if (statusMessageDoc.getMDWStatusMessage().getStatusCode() != 0)
            throw new RemoteException("Error launching process: "
                    + statusMessageDoc.getMDWStatusMessage().getStatusMessage());

        // audit log in separate dao since launch is multi-threaded
        UserActionVO userAction = new UserActionVO(project.getUser().getUsername(), Action.Run,
                processVersion.getActionEntity(), processVersion.getId(),
                processVersion.getLabel());
    ...
    public String launchSynchronousProcess(WorkflowProcess processVersion, String masterRequestId,
            String owner, Long ownerId, List<VariableValue> variableValues, String responseVarName)
            throws RemoteException, DataAccessException, XmlException {
        Map<VariableVO, String> variables = new HashMap<VariableVO, String>();
        for (VariableValue variableValue : variableValues) {
            variables.put(variableValue.getVariableVO(), variableValue.getValue());
        ...
        String ret = restfulServer.launchSynchronousProcess(processVersion.getId(), masterRequestId,
                owner, ownerId, variables, responseVarName, oldFormat);
        dataAccess.auditLog(Action.Run, processVersion);
        return ret;
    }

    public String sendExternalEvent(String request, Map<String, String> headers)
            throws DataAccessException, RemoteException {
        String ret = dataAccess.getDesignerDataAccess().sendMessage("RestfulWebService", request,
                headers);
        dataAccess.auditLog(Action.Send, Entity.Event, new Long(0), "External Event");
        return ret;
    }

    public String notifyProcess(String request, Map<String, String> headers)
            throws DataAccessException, RemoteException {
        String ret = dataAccess.getDesignerDataAccess().sendMessage("RestfulWebService", request,
                headers);
    ...
    public String toggleProcessLock(WorkflowProcess processVersion, boolean lock)
            throws DataAccessException, RemoteException {
        ...
        return dataAccess.getDesignerDataAccess().lockUnlockProcess(processVersion.getId(),
                project.getUser().getUsername(), lock);
    }

    public String toggleWorkflowAssetLock(WorkflowAsset asset, boolean lock)
            throws DataAccessException, RemoteException {
        return dataAccess.getDesignerDataAccess().lockUnlockRuleSet(asset.getId(),
                project.getUser().getUsername(), lock);
    }
    ...
    public void remoteImportVcs() throws DataAccessException {
        try {
            ...
            if (user == null || password == null)
                MessageDialog.openError(MdwPlugin.getShell(), "VCS Import",
                        "Please set repository credentials for " + project.getLabel());
            String encryptedPassword = CryptUtil.encrypt(password);
            dataAccess.getDesignerDataAccess().importFromVcs(user, encryptedPassword,
                    repo.getBranch());
        }
        catch (Exception ex) {
    ...
    public String checkForDbMismatch(ApplicationSummary appSummary) {
        ...
        if (dbInfo != null) {
            ...
            if (!ds.getJdbcUrl().equalsIgnoreCase(dbInfo.getJdbcUrl())) {
                if (!ds.isMariaDb() || !ds.getMariaDbUrlAsMySql().equals(dbInfo.getJdbcUrl())) // server
                                                                                               // may
                                                                                               // report
                                                                                               // as
                                                                                               // mysql
    ...
    public void toggleStubServer() {
        if (StubServer.isRunning()) {
            ...
            try {
                StubServer.start(restfulServer, project.getServerSettings().getStubServerPort(),
                        new UiStubber(), project.isOldNamespaces());
            }
            catch (IOException ex) {
    ...
    private static LogWatcher logWatcher; // TODO allow one log watcher per
                                          // project
