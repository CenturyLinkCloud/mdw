...
public class ProcessCanvasWrapper extends DesignerPanelWrapper implements AWTEventListener
{
  ...
  public void setProcess(WorkflowProcess processVersion) { this.process = processVersion; }

  private WorkflowElement designerCanvasSelection;
  public WorkflowElement getCanvasSelection() { return designerCanvasSelection; }
  ...
  public void setProcessInstancePage(ProcessInstancePage piPage) { this.processInstancePage = piPage; }
  ...
  public ProcessCanvasWrapper(Composite parent, WorkflowProcess processVersion)
  {
    ...
    Toolkit.getDefaultToolkit().addAWTEventListener(this, AWTEvent.MOUSE_EVENT_MASK | AWTEvent.MOUSE_MOTION_EVENT_MASK | AWTEvent.WINDOW_EVENT_MASK | AWTEvent.KEY_EVENT_MASK);
  ...
  public EmbeddedSwingComposite createEmbeddedSwingComposite(Composite parent)
  {
    return new EmbeddedSwingComposite(parent, SWT.NONE)
    {
      ...
      protected JComponent createSwingComponent()
      {
        ...
        FlowchartPage.showtip = !prefsStore.getBoolean(PreferenceConstants.PREFS_DESIGNER_SUPPRESS_TOOLTIPS);
  ...
  {
    ProcessEditor processEditor = (ProcessEditor) MdwPlugin.getActivePage().findEditor(getProcess());
  ...
  protected void refreshCanvas()
  {
    ...
    {
      getFlowchartPage().frame.getDataModel().removeProcessGraph(getFlowchartPage().getProcess());
      getDesignerProxy().loadProcess(getProcess(), getFlowchartPage());
      getFlowchartPage().canvas.repaint();
      updateCanvasBackground();
      getProcess().fireElementChangeEvent(ElementChangeEvent.ChangeType.PROPERTIES_CHANGE, null);
    }
  }
  ...
  public void saveProcess(PersistType persistType, int version)
  {
    ...
    // save the process
    getDesignerProxy().saveProcessWithProgress(getProcess(), getFlowchartPage(), persistType, version, validate, keepLocked);
  ...
  public void cutSelection()
  {
    ...
    {
      // simulate ctrl-x
      //KeyEvent e = new KeyEvent(canvas, KeyEvent.KEY_RELEASED, System.currentTimeMillis(), KeyEvent.CTRL_DOWN_MASK, KeyEvent.VK_X, (char)0x78);
      //getFlowchartPage().canvas.keyPressed(e);

      // since designer canvas has nonstandard mnemonics for ctrl-x, use the following
      copySelection();
      deleteSelection();
    }
  }
  ...
  public void copySelection()
  {
    ...
    {
      // simulate ctrl-c
      KeyEvent e = new KeyEvent(canvas, KeyEvent.KEY_RELEASED, System.currentTimeMillis(), KeyEvent.CTRL_DOWN_MASK, KeyEvent.VK_C, (char)0x63);
  ...
  public void pasteSelection()
  {
    ...
    {
      // simulate ctrl-v
      KeyEvent e = new KeyEvent(canvas, KeyEvent.KEY_RELEASED, System.currentTimeMillis(), KeyEvent.CTRL_DOWN_MASK, KeyEvent.VK_V, (char)0x76);
      getFlowchartPage().canvas.keyReleased(e);
    }
  }
  ...
  public void deleteSelection()
  {
    ...
    if (canvas != null)
    {
      ...
      KeyEvent e = new KeyEvent(canvas, KeyEvent.KEY_PRESSED, System.currentTimeMillis(), 0, KeyEvent.VK_DELETE, (char)0x7f);
      getFlowchartPage().canvas.keyPressed(e);
    }
  }
  ...
  public void eventDispatched(AWTEvent event)
  {
    ...
    if (event.getID() == MouseEvent.MOUSE_RELEASED)
    {
      ...
      if (!isInstance() && !allowInPlaceLabelEditing && (getFlowchartPage().canvas.getAnchor() == -2 || designerCanvasSelection instanceof Note))
    ...
    else if (event.getID() == MouseEvent.MOUSE_PRESSED)
    {
      ...
      if (!isInstance() && !allowInPlaceLabelEditing && getFlowchartPage().canvas.getAnchor() == -2)
        mouseEvent.consume();  // no in-place editing unless specified
    ...
    else if (event.getID() == KeyEvent.KEY_PRESSED && !allowInPlaceLabelEditing)
    {
      ...
      if (keycode != KeyEvent.VK_DELETE && keycode != KeyEvent.VK_UP && keycode != KeyEvent.VK_DOWN
...
//      designerCanvasSelection.removeDirtyStateListener(this);
    ...
    {
      if (isInstance())
        obj = getProcessInstancePage().canvas.objectAt(process, mouseX, mouseY, getProcessInstancePage().canvas.getGraphics());
      else
        obj = getFlowchartPage().canvas.objectAt(process, mouseX, mouseY, getFlowchartPage().canvas.getGraphics());
    }
    else
    {
      obj = process.objectAt(mouseX, mouseY, isInstance() ? getProcessInstancePage().canvas.getGraphics() : getFlowchartPage().canvas.getGraphics());
    }
    ...
    if (obj != null)
    {
      ...
      if (obj instanceof Node)
      {
        ...
        if (isInstance())
        {
          ...
          if (activityInstances.size() == 0 && getProcess().getEmbeddedSubProcessInstances() != null)
          {
            // try embedded subprocess instances
            for (ProcessInstanceVO embeddedSubProcessInstance : getProcess().getEmbeddedSubProcessInstances())
              activityInstances.addAll(embeddedSubProcessInstance.getActivityInstances(activity.getId()));
          }
          ...
          {
            // TODO: load subprocess instances when process instance id loaded (like manual task instances above)
            List<ProcessInstanceVO> subProcessInstances = designerProxy.getSubProcessInstances(getProcess(), activity);
            if (subProcessInstances.size() == 0 && getProcess().getEmbeddedSubProcessInstances() != null)
            {
              for (ProcessInstanceVO embeddedSubProcessInstance : getProcess().getEmbeddedSubProcessInstances())
                subProcessInstances.addAll(designerProxy.getSubProcessInstances(embeddedSubProcessInstance, activity));
            }
  ...
  private Menu buildPopupMenu(final int x, final int y)
  {
    ...
    if (isInstance() && designerCanvasSelection instanceof Activity)
    {
      ...
      {
        if (activity.getSubProcessInstances() != null && !activity.getSubProcessInstances().isEmpty() && activity.getSubProcessInstances().size() < 50)
        {
    ...
    if (isInstance() && designerCanvasSelection instanceof Activity
        ...
        && ((Activity)designerCanvasSelection).getProcess().isUserAuthorized(UserRoleVO.PROCESS_EXECUTION))
    {
    ...
    try
    {
      pbi.busyWhile(new Runnable()
      {
        public void run()
        {
          ...
          String subProcName = subprocessActivity.getAttribute(WorkAttributeConstant.PROCESS_NAME);
          if (subProcName == null)
          {
            MessageDialog.openError(getDisplay().getActiveShell(), "Open Subprocess", "Please select a subprocess");
            return;
          }
          String subProcVer = subprocessActivity.getAttribute(WorkAttributeConstant.PROCESS_VERSION);
          WorkflowProcess subProcess = new AssetLocator(subprocessActivity, AssetLocator.Type.Process).getProcessVersion(new AssetVersionSpec(subProcName, subProcVer));
          if (subProcess == null)
          {
            PluginMessages.uiError(getDisplay().getActiveShell(), "SubProcess not found: " + subProcName + " v" + subProcVer, "Open SubProcess", workflowProject);
            return;
          }
          ...
          {
            PluginMessages.uiError(getDisplay().getActiveShell(), ex, "Open SubProcess", workflowProject);
          }
        }
      });
    }
      ...
      pbi.busyWhile(new Runnable()
      {
        public void run()
        {
          try
          {
            ...
            IStorage storage = new DocumentStorage(project, process.getLabel(), project.getDataAccess().loadRuleSet(process.getId()).getRuleSet());
  ...
  private void openJava(final Activity javaActivity)
  {
    ...
    try
    {
      pbi.busyWhile(new Runnable()
      {
        public void run()
        {
          ArtifactEditorValueProvider artifactValueProvider = new JavaEditorValueProvider(javaActivity);
          ArtifactEditor artifactEditor = new ArtifactEditor(javaActivity, artifactValueProvider, null);
          artifactEditor.openTempFile(new NullProgressMonitor());
        }
      });
    }
    ...
    try
    {
      pbi.busyWhile(new Runnable()
      {
        public void run()
        {
          ...
          {
            PluginMessages.uiError(getDisplay().getActiveShell(), ex, "Open Process", processVersion.getProject());
          }
        }
      });
    }
    ...
    try
    {
      pbi.busyWhile(new Runnable()
      {
        public void run()
        {
          ...
          {
            IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();
            IViewPart viewPart = page.showView("mdw.views.designer.process.instance.list");
            if (viewPart != null)
            {
