...
public class BamEventComposer extends Composer {
    ...
    public BamMessageDefinition getBamMessage() {
        return bamMessage;
    }

    public void setBamMessage(BamMessageDefinition bamMessage) {
        this.bamMessage = bamMessage;
    }
    ...
    public void setElement(WorkflowElement element) {
        ...
        WorkflowAsset bamPageletAsset = getElement().getPackage()
                .getAsset(WorkflowProject.BAM_PAGELET);
        if (bamPageletAsset == null) {
            // try baseline package
            WorkflowPackage baselinePackage = element.getProject()
                    .getPackage(PackageVO.BASELINE_PACKAGE_NAME);
        ...
        try {
            if (bamPageletAsset == null) {
                bamPagelet = PAGELETDocument.Factory.parse(BamEventComposer.DEFAULT_BAM_PAGELET)
                        .getPAGELET();
            }
            else {
                if (!bamPageletAsset.isLoaded())
                    bamPageletAsset.load();
                bamPagelet = PAGELETDocument.Factory.parse(bamPageletAsset.getContent())
                        .getPAGELET();
            }
        }
    ...
    protected AttributesTableContainer createAttributesTable(int width, int colspan,
            TableT bamTable) {
    ...
    private TableT getBamPageletTable() throws BamPageletValidationException {
        ...
        if (tables == null || tables.size() != 1)
            throw new BamPageletValidationException(
                    "Bad BAM Pagelet - Should contain a single TABLE element");
        TableT bamTable = tables.get(0);
        if (AttributesTableContainer.getNameWidget(bamTable) == null)
            throw new BamPageletValidationException(
                    "Bad BAM Pagelet - TABLE should contain a subelement with NAME='name'");
        if (AttributesTableContainer.getValueWidget(bamTable) == null)
            throw new BamPageletValidationException(
                    "Bad BAM Pagelet - TABLE should contain a subelement with NAME='value'");
