...
public class WorkflowAssetEditor extends PropertyEditor {
    ...
    public boolean isTaskTemplate() {
        return assetTypes != null && assetTypes.size() == 1
                && assetTypes.get(0).equals(RuleSetVO.TASK);
    }
    ...
    public WorkflowAssetEditor(WorkflowElement workflowElement, String attributeName,
    ...
    public WorkflowElement getWorkflowAsset() {
        return workflowAsset;
    }

    public void setWorkflowAsset(WorkflowElement asset) {
                ...
                if (selItems.length == 1) {
                    ...
                    if (selItem.getParentItem() == null) {
                        ...
                        if (pkg == null) {
                            ...
                            AssetLocator assetLocator = new AssetLocator(getElement(),
                                    getLocatorType());
                            workflowAsset = assetLocator.getAssetVersion(selItem.getText(), pkg);
                        }

                        link.setText(getLinkLabel());
                        fireValueChanged(new AssetLocator(getElement(), getLocatorType())
                                .attrFromAsset(workflowAsset));
            ...
            public void handleEvent(Event event) {
                if (treeCombo.getSelection().length == 0) {
                    if (!suppressFire) {
                        ...
                        workflowAsset = null;
                        link.setText("");
                        AssetVersionSpec versionSpec = AssetVersionSpec
                                .parse(treeCombo.getText().trim());
                        ...
                        boolean verChanged = oldVer == null ? newVer != null && newVer.length() > 0
                                : !oldVer.equals(newVer);
                        if (verChanged)
    ...
    private void updateVersionAttribute(WorkflowElement workflowAsset) {
        ...
        if (isProcess()) {
            if (getProject().checkRequiredVersion(5, 5) && !(getElement() instanceof Activity
                    && ((Activity) getElement()).isOldMultipleSubProcInvoke())) {
                if (MdwPlugin.getSettings().isInferSmartSubprocVersionSpec())
                    versionAttr = AssetVersionSpec
                            .getDefaultSmartVersionSpec(versionSpec.getVersion());
                else
                    versionAttr = versionSpec.getVersion();
            }
            else {
                versionAttr = String.valueOf(((WorkflowProcess) workflowAsset).getVersion()); // pre-5.5
                                                                                              // compatibility
            }

            getElement().setAttribute(WorkAttributeConstant.PROCESS_VERSION, versionAttr);
        }
        else if (getProject().checkRequiredVersion(5, 5)) // assets only save
                                                          // version for 5.5
        {
            if (MdwPlugin.getSettings().isInferSmartSubprocVersionSpec())
    ...
    private void updateVersionAttribute(String version) {
        ...
        else if (getProject().checkRequiredVersion(5, 5)) // assets only saved
                                                          // for 5.5
    ...
    public void setValue(Activity activity) {
        ...
        if (attributeValue == null && isTaskTemplate()) {
            ...
            if (taskName != null) {
                ...
                for (TaskTemplate template : activity.getProcess().getPackage()
                        .getTaskTemplates()) {
    ...
    public void setValue(String value) {
        ...
        if (versionAttr != null && attrVal != null && !attrVal.isEmpty()) {
            if (!versionAttr.equals(AssetVersionSpec.VERSION_LATEST)) // special
                                                                      // case
            {
                try {
                    // old style
        ...
        // widgets can be null here when invoked from
        ...
        if (treeCombo != null) {
            if (workflowAsset != null) {
                ...
                for (CTreeComboItem pkgItem : treeCombo.getItems()) {
                    if (((pkgItem.getText().isEmpty()
                            || pkgItem.getText().equals(PackageVO.DEFAULT_PACKAGE_NAME))
                            && workflowAsset.getPackage() == null)
                        ...
                        for (CTreeComboItem assetItem : pkgItem.getItems()) {
                            if (assetItem.getText().equals(workflowAsset.getName())
                                    || assetItem.getText().equals(workflowAsset.getLabel())) {
                                selItem = assetItem;
                                break;
                            }
                        }
    ...
    private String getLinkLabel() {
        if (workflowAsset == null) {
            ...
            boolean lockedToUser = (workflowAsset instanceof WorkflowProcess)
                    ? ((WorkflowProcess) workflowAsset).isLockedToUser()
                    : ((WorkflowAsset) workflowAsset).isLockedToUser();
            if (lockedToUser)
    ...
    private void openWorkflowAsset() {
        if (workflowAsset instanceof WorkflowProcess) {
            ...
            try {
                ...
                TaskTemplateEditor editor = (TaskTemplateEditor) page.openEditor(editorInput,
                        "mdw.editors.taskTemplate");
    ...
    private WorkflowElement createWorkflowAsset() {
        if (isProcess()) {
            ...
            wizard.init(workbench,
                    new StructuredSelection(new Object[] { getElement().getPackage() }));
