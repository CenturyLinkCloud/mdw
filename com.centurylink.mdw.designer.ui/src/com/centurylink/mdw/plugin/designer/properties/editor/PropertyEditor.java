...
public class PropertyEditor
{
  ...
  public FontData getFont() { return font; }
  public void setFont(FontData font) { this.font = font; }
  ...
  public int getVerticalIndent() { return verticalIndent; }
  public void setVerticalIndent(int vi) { this.verticalIndent = vi; }
  ...
  public String getScriptLanguages() { return scriptLanguages; }
  ...
  public boolean isFireDirtyStateChange() { return fireDirtyStateChange; }
  public void setFireDirtyStateChange(boolean fire) { this.fireDirtyStateChange = fire; }

  public void setValue(Object value)
  {
    if (valueConverter == null)
      throw new UnsupportedOperationException("Requires a value converter for: " + Object.class);
  ...
  public void setValue(WorkflowAsset asset)
  {
    if (mbengNode != null && !type.equals(TYPE_LINK) && !type.equals(TYPE_INFO))
    {
      ...
      {
        String unitsAttrName = mbengNode.getAttribute("UNITS_ATTR");
        String unitsAttr = unitsAttrName == null ? Units.Seconds.toString() : asset.getAttribute(unitsAttrName);
  ...
  public Units[] getAcceptedUnits() { return acceptedUnits; }
  public void setAcceptedUnits(Units[] accepted) { this.acceptedUnits = accepted; }
  ...
  public List<Image> getImageOptions() { return imageOptions; }
  ...
  public MbengNode getMbengNode() { return mbengNode; }

  private Control widget;
  public Control getWidget() { return widget; }
  public void setWidget(Control widget) { this.widget = widget; }

  private Label labelWidget;
  public Label getLabelWidget() { return labelWidget; }

  private Label commentWidget;
  public Label getCommentWidget() { return commentWidget; }
  ...
  public void updateWidget(String value)
  {
    ...
    if (type.equals(TYPE_TEXT))
      ...
      ((Spinner)widget).setSelection(newValue == null || newValue.length() == 0 ? 0 : Integer.parseInt(newValue));
  ...
  public PropertyEditor(WorkflowElement workflowElement, MbengNode mbengNode)
  {
    ...
    if (getType().equals(TYPE_COMBO) || getType().equals(TYPE_PICKLIST))
    {
      if (source == null)
      {
        ...
        for (MbengNode optionNode = mbengNode.getFirstChild(); optionNode != null; optionNode = optionNode.getNextSibling())
        {
          valueOptions.add(optionNode.getValue());
          if (optionNode.getAttribute("PARAMETER") != null || type.equals(TYPE_PARAMETERIZED_COMBO))
          {
            type = TYPE_PARAMETERIZED_COMBO;
            if (valueConverter == null)
              valueConverter = new ParameterizedValueConverter((AttributeHolder)workflowElement);
            ...
            ComboParameter comboParam = parameterizedValueConverter.new ComboParameter(paramName, paramSource == null ? null : optionNode.getAttribute("TYPE"));
      ...
      {
        valueOptions = new ArrayList<String>();  // to be populated dynamically
      }
      ...
      {
        if (!(workflowElement instanceof Activity))
          throw new PropertyEditorException("Activities source only valid for Activity element.");

        Activity activity = (Activity) workflowElement;
        valueOptions = activity.getUpstreamActivityNames();
        // hard-wire the attribute value since synced activities are read-only
      ...
      else if (source.equals("Variables") || source.equals("DocumentVariables"))
      {
        ...
        if (process == null)
          throw new PropertyEditorException("Variables source only valid for Process, Activity or Transition elements.");
  ...
  /**
   * Creates the operating system resources for the widget specific
   * to the type of this property editor.  Note: Rendering for Script/RULE
   * type property editors are handled in the ScriptSection.
   */
  ...
  private Text createTextInput(Composite parent)
  {
    ...
    textField.addModifyListener(new ModifyListener()
    {
      public void modifyText(ModifyEvent e)
      {
        ...
        boolean changed = oldValue == null ? value != null && value.length() > 0 : !oldValue.replace("\r", "").equals(value.replace("\r", ""));
        if (changed)
    ...
    comboBox.addModifyListener(new ModifyListener()
      {
        public void modifyText(ModifyEvent e)
        {
          ...
          boolean changed = oldValue == null ? value != null && value.length() > 0 : !oldValue.equals(value);
          if (changed)
            fireValueChanged(value);
        }
      });
  ...
  private ImageCombo createImageCombo(Composite parent)
  {
    if (valueOptions == null || imageOptions == null)
      throw new PropertyEditorException("Image Combo control requires value options and image options.");
      ...
      public void widgetSelected(SelectionEvent e)
      {
        if (getValue() != null)
        {
          if (getValue().startsWith("/MDWWeb/") || getValue().startsWith("/MDWHub/"))
          {
            try
            {
              String docPath = Compatibility.getDocumentationPath(getValue().substring(7));
  ...
  private ListComposer createPickList(Composite parent)
  {
    ...
    if (isReadOnly())
      style = style | SWT.READ_ONLY;
    final ListComposer pickList = new ListComposer(parent, style, title, valueOptions, 200, isReadOnly());
    GridData gd = new GridData(SWT.LEFT);
    gd.horizontalSpan = COLUMNS - 1;
  ...
  private ParameterizedCombo createParameterizedCombo(Composite parent)
  {
    ...
    final ParameterizedCombo composer = new ParameterizedCombo(parent, getElement(), (ParameterizedValueConverter)valueConverter, style, width, isReadOnly());
  ...
  private TimeInterval createTimer(Composite parent)
  {
    ...
    boolean allowExpressions = "TIMER_WAIT".equals(getName()); // ug

    final TimeInterval timer = new TimeInterval(parent, style, interval, units, 75, allowExpressions, acceptedUnits);
    ...
    timer.addModifyListener(new ModifyListener()
      {
        public void modifyText(ModifyEvent e)
        {
          ...
          boolean valueChanged = oldValue == null || oldValue.equals("0") ? value != null && value.length() > 0 && !value.equals("0") : !oldValue.equals(value);
          boolean unitsChanged = oldUnits == null ? units != null && units.toString().length() > 0 : !oldUnits.equals(units);
  ...
  public void fireValueChanged(Object newValue, boolean fireDirtyStateChanged)
  {
    ...
    {
      ValueChangeListener listener = (ValueChangeListener) valueChangeListeners.getListeners()[i];
