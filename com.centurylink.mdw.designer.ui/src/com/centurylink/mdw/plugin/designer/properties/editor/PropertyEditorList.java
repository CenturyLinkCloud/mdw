...
public class PropertyEditorList implements Iterable<PropertyEditor>
{
  ...
  public PropertyEditorList(WorkflowElement workflowElement, String attrXml) throws PropertyEditorException
  {
    ...
    try
    {
      ...
      for (currentNode = mbengDocument.getRootNode().getFirstChild(); currentNode != null; currentNode = currentNode.getNextSibling())
      {
        ...
        else if (type.equals(PropertyEditor.TYPE_PICKLIST) && currentNode.getAttribute("SOURCE") == null && currentNode.getFirstChild() == null)
        {
          currentNode.setName(TableEditor.TYPE_TABLE);  // substitute table with one column
                       ...
                       || "TaskTemplates".equals(currentNode.getAttribute("SOURCE"))))
        {
          ...
          if (propEditor.getValue() != null && propEditor.getValue().equals(currentNode.getAttribute("DEFAULT")))
            propEditor.setValue("");  // don't set default value for rulesets
        }
        else if (type.equals(PropertyEditor.TYPE_COMBO) && "ProcessVersion".equals(currentNode.getAttribute("SOURCE")))
        {
          propEditor = null;  // this field is suppressed now (populated through asset picker)
        }
        else
        {
        ...
        if (propEditor != null
            && !propEditor.getType().equals(PropertyEditor.TYPE_LINK)
            ...
            && !propEditor.getType().equals(TableEditor.TYPE_TABLE))
        {
          ...
          final String unitsAttr = unitsAttrVal == null ? WorkAttributeConstant.SLA_UNIT : unitsAttrVal;
          ...
          propEditor.addValueChangeListener(new ValueChangeListener()
          {
            public void propertyValueChanged(Object newValue)
            {
              ...
              if (newValue instanceof TimeInterval.TimerValue)
              {
                ...
                workflowElement.setAttribute(attrName, timerValue.getInterval().equals("0") ? null : timerValue.getInterval());
                workflowElement.setAttribute(unitsAttr, timerValue.getUnits().toString());
                if (!workflowElement.getProject().isMdw5() && workflowElement instanceof Activity && ((Activity)workflowElement).isEventWait())
                  ((Activity)workflowElement).getNode().nodet.setSla(timerValue.getSeconds());
              }
              ...
              else if ("TaskCategory".equals(attrSource) && newValue != null)
              {
                ...
                if (sep > 0)
                  workflowElement.setAttribute(attrName, newVal.substring(0, sep));
                else
