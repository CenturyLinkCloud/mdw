...
public class PropertySection extends org.eclipse.ui.views.properties.tabbed.AbstractPropertySection
{
  ...
  public WorkflowElement getSelection() { return selection; }
  ...
  public void createControls(Composite parent, org.eclipse.ui.views.properties.tabbed.TabbedPropertySheetPage tabbedPropertySheetPage)
  {
  ...
  public int getTopMargin() { return 6; }
  ...
  public void setInput(IWorkbenchPart part, ISelection selection)
  {
    ...
    if (selection != null && !selection.equals(this.selection) && this.selection.isOverrideAttributeDirty(getOverrideAttributePrefix()) && !warnedDirty)
      warnDirty();
  ...
  {
    if (!tabbedPropertySheetPage.getControl().isDisposed() && Thread.currentThread().equals(MdwPlugin.getDisplay().getThread()))
  ...
  protected void saveOverrideAttributes()
  {
    BusyIndicator.showWhile(getShell().getDisplay(), new Runnable()
    {
      public void run()
      {
        ...
        boolean saved = getDesignerProxy().saveOverrideAttributes(getOverrideAttributePrefix(), OwnerType.PROCESS, selection.getProcessId(),
            subType, subType == null ? null : String.valueOf(selection.getId()), getOverrideAttributes());
        if (saved)
          setDirty(false);
      }
    });
  ...
  {
    org.eclipse.ui.internal.views.properties.tabbed.view.TabbedPropertyComposite composite =
        (org.eclipse.ui.internal.views.properties.tabbed.view.TabbedPropertyComposite)tabbedPropertySheetPage.getControl();
    if (composite != null && !composite.isDisposed())
    {
      ...
      if (tabs != null && !tabs.isDisposed())
      {
        for (int i = 0; i < tabs.getNumberOfElements(); i++)
        {
          ...
          {
            org.eclipse.ui.internal.views.properties.tabbed.view.TabbedPropertyList.ListElement tabElem
                = (org.eclipse.ui.internal.views.properties.tabbed.view.TabbedPropertyList.ListElement) elem;
            ITabItem tabItem = tabElem.getTabItem();
            // by convention, Pagelet tabs and tab ids ending in ".override" are for override attributes
            if (tabItem instanceof PageletTab)
            {
              PageletTab pageletTab = (PageletTab) tabItem;
              boolean wasDirty = pageletTab.isDirty();
              boolean isDirty = selection.isOverrideAttributeDirty(pageletTab.getOverrideAttributePrefix());
              if (wasDirty != isDirty)
              {
            ...
            else if (tabItem instanceof PropertyTab && tabItem.toString().endsWith(".override"))
            {
              ...
              boolean isDirty = selection.isOverrideAttributeDirty(propertyTab.getOverrideAttributePrefix());
              if (wasDirty != isDirty)
              {
