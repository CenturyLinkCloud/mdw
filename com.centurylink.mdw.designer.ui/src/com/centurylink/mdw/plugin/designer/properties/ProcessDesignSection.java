...
public class ProcessDesignSection extends PropertySection implements IFilter
{
  ...
  public void drawWidgets(Composite composite, WorkflowElement selection)
  {
    ...
    startPagePropertyEditor = new WorkflowAssetEditor(process, WorkAttributeConstant.PROCESS_START_PAGE, Arrays.asList(new String[] {RuleSetVO.FACELET}));
  ...
  private List<String> getPerformanceLevelOptions()
  {
    if (perfLevelOptions == null)
    {
      ...
      perfLevelOptions.add("3 - Read cache variables/documents (multi-message-per-transition for regular process)");
      perfLevelOptions.add("5 - Cache only for variable/documents (for service process only)");
