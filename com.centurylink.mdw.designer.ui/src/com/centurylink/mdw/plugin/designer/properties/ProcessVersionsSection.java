...
public class ProcessVersionsSection extends PropertySection
    ...
    private void openProcess(WorkflowProcess processVersion) {
        ...
        try {
            ...
            PluginMessages.uiError(composite.getShell(), ex, "Open Process",
    ...
    private Menu createContextMenu(Shell shell) {
        ...
        if (!selection.isEmpty() && !process.getProject().isProduction()
                ...
                && (selection.size() == 1
                        && ((WorkflowProcess) selection.getFirstElement()).isLatest())
            ...
            deleteItem.addSelectionListener(new SelectionAdapter() {
                public void widgetSelected(SelectionEvent e) {
                    if (selection.size() >= 1
                            && selection.getFirstElement() instanceof WorkflowProcess) {
