...
public class TransitionSection extends PropertySection
{
  ...
  public void setSelection(WorkflowElement selection)
  {
    ...
    if (transition.getDelay() != null && transition.getDelay().endsWith("s"))
      delayPropertyEditor.setValue(transition.getDelay().substring(0, transition.getDelay().length() - 1), TimeInterval.Units.Seconds);
    else if (transition.getDelay() != null && transition.getDelay().endsWith("h"))
      delayPropertyEditor.setValue(transition.getDelay().substring(0, transition.getDelay().length() - 1), TimeInterval.Units.Hours);
    else
    ...
    controlPointsPropertyEditor.setEditable(!transition.isReadOnly() && !transition.sameFromAndTo());
  }
