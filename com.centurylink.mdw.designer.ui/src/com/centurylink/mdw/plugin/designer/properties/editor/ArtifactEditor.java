...
/**
 * Provides the capability to edit artifacts using the eclipse editor associated
 * with the artifact's filename extension.
 */
public class ArtifactEditor extends PropertyEditor {
    ...
    public ArtifactEditorValueProvider getValueProvider() {
    ...
    public ArtifactEditor(WorkflowElement canvasSelection,
            ArtifactEditorValueProvider valueProvider, String label) {
    ...
    public void openTempFile(IProgressMonitor monitor) {
        ...
        try {
            ...
            if (!folder.exists())
                PluginUtil.createFoldersAsNeeded(getElement().getProject().getSourceProject(),
                        folder, monitor);
                                ...
                                if (tempFileEditor != null) {
                                    ...
                                    tempFileEditor.addPropertyListener(new IPropertyListener() {
                                        public void propertyChanged(Object source, int propId) {
                                            if (source instanceof EditorPart
                                                    && propId == IWorkbenchPartConstants.PROP_DIRTY) {
                                                if (((EditorPart) source).isDirty())
                                                    fireValueChanged(null, true); // process
                                                                                  // editor
                                                                                  // should
                                                                                  // show
                                                                                  // dirty
                                    ...
                                    ArtifactResourceListener resourceListener = new ArtifactResourceListener(
                                            getElement(), valueProvider, file);
                                    ...
                                    PlatformUI.getWorkbench().addWorkbenchListener(
                                            new ArtifactEditorWorkbenchListener(tempFileEditor));
                                }
