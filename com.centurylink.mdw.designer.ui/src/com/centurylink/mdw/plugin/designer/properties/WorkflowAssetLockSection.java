...
public class WorkflowAssetLockSection extends PropertySection implements IFilter, ElementChangeListener
{
  ...
  public void setSelection(WorkflowElement selection)
  {
    ...
    lockButtonPropertyEditor.setEnabled(workflowAsset.isLockedToUser()
        || (workflowAsset.getLockingUser() == null && workflowAsset.isUserAuthorized(UserRoleVO.ASSET_DESIGN)));
  }
  ...
  private void toggleLock()
  {
    ...
    BusyIndicator.showWhile(getShell().getDisplay(), new Runnable()
    {
      public void run()
      {
        try
        {
          String lockMessage = workflowAsset.getProject().getDesignerProxy().toggleWorkflowAssetLock(workflowAsset, lock);
    ...
    if (errorMessage.toString().length() > 0)
      PluginMessages.uiError(errorMessage.toString(), "Lock/Unlock Resource", workflowAsset.getProject());
  }
}