...
public abstract class ArtifactEditorValueProvider
{
  ...
  {
    return getProject().getSourceProject().getFolder(MdwPlugin.getSettings().getTempResourceLocation());
  }
  ...
  public String getBundleSymbolicName()
  {
    ...
    try
    {
      ...
      if (!buildFile.exists())
        buildFile = new MavenBuildFile(getProject().getSourceProject());  // fall back to pom.xml
      if (buildFile.exists())
      {
      ...
      {
        PluginMessages.log("Neither build.gradle nor pom.xml was found for determining bundle symbolic name");
      }
    }
    catch (Exception ex)
    {
