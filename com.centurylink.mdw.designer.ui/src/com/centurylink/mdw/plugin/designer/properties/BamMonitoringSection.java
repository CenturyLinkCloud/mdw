...
public class BamMonitoringSection extends PropertySection implements IFilter {
    ...
    public void setSelection(WorkflowElement element) {
        ...
        bamEventDefs.put(WorkAttributeConstant.BAM_START_MSGDEF,
                getValue(WorkAttributeConstant.BAM_START_MSGDEF));
        bamEventDefs.put(WorkAttributeConstant.BAM_FINISH_MSGDEF,
                getValue(WorkAttributeConstant.BAM_FINISH_MSGDEF));
        ...
        if (element.overrideAttributesApplied())
            ...
            helpPropertyEditor.setLabel(
                    "Attributes unavailable. Reload process with server online. (<A>Attributes Help</A>)");
    ...
    public void drawWidgets(Composite composite, WorkflowElement selection) {
        ...
        triggerPropertyEditor.addValueChangeListener(new ValueChangeListener() {
            public void propertyValueChanged(Object newValue) {
                ...
                bamMessagePropertyEditor.setEditable(isEditable()); // otherwise
                                                                    // reverts
                                                                    // Add/Delete
                                                                    // buttons
                                                                    // to
        ...
        bamMessagePropertyEditor.addValueChangeListener(new ValueChangeListener() {
            public void propertyValueChanged(Object newValue) {
                ...
                element.setAttribute(selectedTrigger,
                        valueConverter.toPropertyValue(bamMessageDef));
                setDirty(true);
            }
        });
    ...
    private boolean isEditable() {
        if (element instanceof WorkflowProcess) {
            ...
            return act.isUserAuthorized(UserRoleVO.ASSET_DESIGN)
                    && act.getProcess().overrideAttributesApplied();
        }
        else if (element instanceof Transition) {
            Transition trans = (Transition) element;
            return trans.isUserAuthorized(UserRoleVO.ASSET_DESIGN)
                    && trans.getProcess().overrideAttributesApplied();
        }
        else {
            return false;
        }
    ...
    private BamMessageDefinition getValue(String attribute) {
        ...
        if (attrVal == null)
            return new BamMessageDefinition(
                    attribute.equals(WorkAttributeConstant.BAM_START_MSGDEF));
        else
    ...
    protected void saveOverrideAttributes() {
        ...
        try {
            ...
            PluginMessages.uiError(getShell(), ex, "Save Monitoring Attributes",
                    element.getProject());
        }
    }
        ...
        if (attributes != null) {
            for (AttributeVO attr : attributes) {
                if (attr.getAttributeName() != null && !attr.getAttributeName().trim().isEmpty()
                        ...
                        && !attr.getAttributeValue().trim().isEmpty()) {
                    attrs.add(attr);
                }
            }
        }
    ...
    public boolean select(Object toTest) {
        if (toTest instanceof Activity || toTest instanceof WorkflowProcess) {
            ...
            if (element instanceof WorkflowProcess
                    && !((WorkflowProcess) element).hasInstanceInfo()) {
