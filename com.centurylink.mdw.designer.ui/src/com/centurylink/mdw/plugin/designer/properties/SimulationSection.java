...
public class SimulationSection extends PropertySection implements IFilter {
    ...
    public void setSelection(WorkflowElement selection) {
        ...
        boolean editable = activity.isUserAuthorized(UserRoleVO.PROCESS_EXECUTION)
                && activity.getProcess().overrideAttributesApplied();
        String stubMode = activity.getAttribute(WorkAttributeConstant.SIMULATION_STUB_MODE);
        if (stubMode == null)
            simPropertyEditor.setValue("Off");
        else
            simPropertyEditor
                    .setValue(activity.getAttribute(WorkAttributeConstant.SIMULATION_STUB_MODE));
    ...
    public void drawWidgets(Composite composite, WorkflowElement selection) {
        ...
        simPropertyEditor.addValueChangeListener(new ValueChangeListener() {
            public void propertyValueChanged(Object newValue) {
                activity.setAttribute(WorkAttributeConstant.SIMULATION_STUB_MODE,
                        (String) newValue);
                setDirty(true);
            }
        });
    ...
    public boolean select(Object toTest) {
        ...
        if (!activity.getProject().checkRequiredVersion(5, 2) || activity.isForProcessInstance()
                || !activity.getProcess().isInRuleSet())
            return false;
    ...
    public void updateAttributes(List<SimulationResponseVO> srVOs) {
        ...
        // first off, identify all the attributes to remove so the correct ones
        // can be re-added
        List<AttributeVO> attrs = activity.getAttributes();
    ...
    class SimulationResponseVO {
        ...
        private void createAttributeValue() {
            this.attributeValue = this.returnCode + "," + Integer.toString(this.chance) + ","
                    + this.response;
        }
    }
