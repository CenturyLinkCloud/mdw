...
public class AssetLocator {
    ...
    public boolean isProcess() {
        return assetType == Type.Process;
    }

    public boolean isTaskTemplate() {
        return assetType == Type.TaskTemplate;
    }
    ...
    public WorkflowElement assetFromAttr(String attrValue) {
        ...
        if (isProcess()) {
            ...
            if (slashIdx <= 0) {
                ...
                WorkflowProcess proc = getElement().getPackage() == null ? null
                        : getProcessVersion(attrValue, getElement().getPackage().getProcesses());
                if (proc != null)
                    return proc;
                else
            ...
            if (pkg == null)
                ...
                return getTaskTemplateVersion(attrValue.substring(slashIdx + 1),
                        pkg.getTaskTemplates());
        }
        else {
            int slashIdx = attrValue.indexOf('/');
            if (slashIdx <= 0) {
                // prefer asset in same package
                WorkflowAsset pkgDoc = getElement().getPackage() == null ? null
                        : getWorkflowAssetVersion(attrValue, getElement().getPackage().getAssets());
                if (pkgDoc != null)
                    return pkgDoc;
                else
                ...
                if (pkg == null)
                    ...
                    return getWorkflowAssetVersion(attrValue.substring(slashIdx + 1),
                            pkg.getAssets());
            }
        }
    }
    ...
    private WorkflowAsset getWorkflowAssetVersion(String versionSpec, List<WorkflowAsset> assets) {
        ...
        for (WorkflowAsset asset : assets) {
            if (asset.getName().matches(spec.getName())) {
                ...
                else if ((AssetVersionSpec.VERSION_LATEST.equals(spec.getVersion())
                        || (isDefaultSmart && asset.meetsVersionSpec(spec.getVersion())))
    ...
    private TaskTemplate getTaskTemplateVersion(String versionSpec,
            ...
            if (taskTemplate.getName().matches(spec.getName())) {
                if (taskTemplate.getVersionString().equals(spec.getVersion())) {
                    ...
                    break;
                        ...
                        || (isDefaultSmart && taskTemplate.meetsVersionSpec(spec.getVersion())))
    ...
    public WorkflowProcess getProcessVersion(AssetVersionSpec spec) {
        if (spec.getPackageName() != null)
            return getProcessVersion(spec.getName() + " v" + spec.getVersion(),
                    getProject().getAllProcessesByPackage(spec.getPackageName()));
        return getProcessVersion(spec.getName() + " v" + spec.getVersion(),
                getProject().getAllProcesses());
    }
    ...
    public TaskTemplate getTaskTemplateVersion(AssetVersionSpec spec) {
        ...
        return getTaskTemplateVersion(spec.getName() + " v" + spec.getVersion(),
                pkg.getTaskTemplates());
    }

    public WorkflowAsset getWorkflowAssetVersion(AssetVersionSpec spec) {
        return getWorkflowAssetVersion(spec.getName() + " v" + spec.getVersion(),
                getProject().getAllWorkflowAssets());
    }
            ...
            if (process.getName().equals(spec.getName())) {
                if (process.getVersionString().equals(spec.getVersion())) {
                    ...
                    break;
                        ...
                        || (isDefaultSmart && process.meetsVersionSpec(spec.getVersion())))
    ...
    private boolean isDefaultSmartFormat(String versionSpec) {
        return versionSpec.startsWith("[") && versionSpec.indexOf(",") > 0
