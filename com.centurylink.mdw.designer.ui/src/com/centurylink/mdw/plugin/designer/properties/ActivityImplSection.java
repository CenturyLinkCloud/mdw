...
public class ActivityImplSection extends PropertySection {
    ...
    private void launchImplClassNameDialog() {
        MdwInputDialog classNameDialog = new MdwInputDialog(getShell(),
                "Fully-Qualified Implementor Class Name", false);
            ...
            if (newClassName.length() > 0) {
                BusyIndicator.showWhile(getShell().getDisplay(), new Runnable() {
                    public void run() {
                        ...
                        activityImpl.getProject().setActivityImplClass(
                                activityImpl.getImplClassName(), activityImpl);
