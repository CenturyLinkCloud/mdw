...
public class TaskNoticesSection extends DesignSection implements IFilter
{
  ...
  private void initializeNotices(Activity activity)
  {
    ...
    {
      attrVal = activity.getProject().checkRequiredVersion(5, 5) ? TaskVO.getDefaultNotices() : TaskVO.getDefaultCompatibilityNotices();
  ...
  class NoticesValueChangeListener extends AttributeValueChangeListener
  {
    ...
    {
      int columnCount = StringHelper.delimiterColumnCount(newValue.substring(0, newValue.indexOf(";")), ",", "\\,"); // to maintain compatibility for Notices with/without asset version
