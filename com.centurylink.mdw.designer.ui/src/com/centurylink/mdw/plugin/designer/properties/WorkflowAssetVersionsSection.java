...
public class WorkflowAssetVersionsSection extends PropertySection implements IFilter, ElementChangeListener
{
  ...
  private Menu createContextMenu(Shell shell)
  {
    ...
    if (!selection.isEmpty() && !workflowAsset.getProject().isProduction()
        ...
        && (selection.size() == 1 && ((WorkflowAsset)selection.getFirstElement()).isLatestVersion() || MdwPlugin.getSettings().isAllowDeleteArchivedProcesses()))
    {
  ...
  public void elementChanged(ElementChangeEvent ece)
  {
    if (ece.getElement().equals(workflowAsset))
    {
      ...
      if (type == ChangeType.VERSION_CHANGE || type == ChangeType.PROPERTIES_CHANGE || type == ChangeType.SETTINGS_CHANGE
          || type == ChangeType.ELEMENT_DELETE || type == ChangeType.ELEMENT_CREATE)
      {
        notifyLabelChange();
        tableEditor.setElement(workflowAsset);
        setTable();
