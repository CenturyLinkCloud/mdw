...
public class ActivityPage extends CodeGenWizardPage {
    ...
    private void createAttrXmlControls(Composite parent, int ncol) {
        ...
        if (getWizard() instanceof StartActivityWizard) {
            ...
            ActivityImpl startImpl = workflowProject == null ? null
                    : workflowProject.getActivityImpl(baseStartClass);
    ...
    protected boolean isPageValid() {
        ...
        return getProject() != null && getPackage() != null && checkString(getActivity().getLabel())
    ...
    public IStatus[] getStatuses() {
        ...
        if (getProject() != null) // is null before inference
        {
            if (getPackage() == null)
                ...
                msg = "An activity implementor named '" + getActivity().getLabel()
                        + "' already exists for project " + getProject().getLabel();
            else if (getProject().activityImplClassExists(
                    getActivity().getJavaPackage() + "." + getActivity().getClassName()))
                msg = "An activity implementor with class '" + getActivity().getJavaPackage() + "."
                        + getActivity().getClassName() + "' already exists for project "
                        + getProject().getLabel();
    ...
    /**
     * Currently only validate for well-formed XML since we have many existing
     * activities with Pagelet definitions that do not pass XSD validation.
     */
