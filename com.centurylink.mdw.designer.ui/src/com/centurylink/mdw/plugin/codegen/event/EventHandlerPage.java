...
public class EventHandlerPage extends CodeGenWizardPage
{
  ...
  public IStatus[] getStatuses()
  {
    ...
    else if (getEventHandler().getProject().externalEventMessagePatternExists(getEventHandler().getMessagePattern()))
      msg = "An event handler with message pattern '" + getEventHandler().getMessagePattern() + "' already exists";

    if (msg == null && !getEventHandler().isCustom())
    {
      if (getEventHandlerWizard().getHandlerAction() == null)
        msg = "ERROR: Missing built-in handler action";  // should never happen
      else if (getEventHandlerWizard().getHandlerAction() == HandlerAction.launchProcess && !checkString(getEventHandler().getProcess()))
