...
public class Generator
{
  ...
  public void createFileAndOpen(Shell shell, JetAccess jet, IProgressMonitor monitor)
    throws CoreException
  {
    IContainer container = jet.findOrCreateContainer(monitor, jet.getConfig().getTargetFolder(), jet.getConfig().getPackageName());
    ...
    {
      if (!(MessageDialog.openQuestion(shell, "Confirm Overwrite", "Target file :'" + targetFile.getLocation() + "' exists.  Overwrite?")))
        return;
    }
   ...
   * @throws CoreException
   */
  public IFile createFile(JetAccess jet, IProgressMonitor monitor)
    throws CoreException
  {
    try
    {
   ...
   * @throws CoreException
   */
  public IFile appendFile(JetAccess jet, IProgressMonitor monitor)
    throws CoreException
  {
    try
    {
  ...
  {
    return new CoreException(new Status(Status.ERROR, MdwPlugin.getPluginId(),
        Status.ERROR, "ERROR: " + ex, ex));   
  }

  /**
   * Sets the MDWCommon.jar file as CLASSPATH variable in Eclipse.  Needed
   * so that the JET Emitter framework can find MDWCommon at runtime.
   */
