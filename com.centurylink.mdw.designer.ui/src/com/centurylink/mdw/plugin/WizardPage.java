...
public abstract class WizardPage extends NewClassWizardPage {
    ...
    public List<WorkflowProject> getProjects() {
        return workflowProjects;
    }

    public void setProjects(List<WorkflowProject> projects) {
    ...
    public void setProject(WorkflowProject wp) {
        workflowProject = wp;
    }
    ...
    protected void createWorkflowProjectControls(Composite parent, int ncol,
            final boolean handleFieldChanged) {
        ...
        workflowProjectCombo.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                if (workflowPackageCombo != null) {
                    ...
                    for (WorkflowPackage packageVersion : workflowProject.getProject()
    ...
    protected void createWorkflowPackageControls(Composite parent, int ncol,
        ...
        workflowPackageCombo.removeAll();
        if (getElement().getProject() != null) {
            for (WorkflowPackage packageVersion : getElement().getProject()
                    .getTopLevelUserVisiblePackages())
                workflowPackageCombo.add(packageVersion.getName());
        }
        ...
        if (getElement().getPackage() != null)
            ...
            workflowPackageCombo.select(0);
        getElement().setPackage(getElement().getProject().getTopLevelUserVisiblePackages()
                .get(workflowPackageCombo.getSelectionIndex()));
    }
    ...
    public void determinePackageFragmentRoot(WorkflowProject workflowProject) {
        ...
        if (oldPackageFragmentRoot == null
                || !oldPackageFragmentRoot.equals(getPackageFragmentRoot()))
    ...
    private void doStatusUpdate() {
        ...
        if (getStatuses() == null) {
            statuses = new IStatus[] {
                    new Status(IStatus.OK, "not_used", 0, getDescription(), null) };
        }
        else {
            statuses = getStatuses();
        }

        // most severe status will be displayed and the ok button
        // enabled/disabled
    ...
    protected void initContainerPage(IJavaElement elem) {
        ...
        if (elem != null) {
            initRoot = org.eclipse.jdt.internal.corext.util.JavaModelUtil
                    .getPackageFragmentRoot(elem);
            try {
                        ...
                        if (jproject.exists()) {
                            ...
                            for (int i = 0; i < roots.length; i++) {
                                if (roots[i].getKind() == IPackageFragmentRoot.K_SOURCE) {
                                    if (roots[i].getElementName().equals(
                                            MdwPlugin.getSettings().getTempResourceLocation())) {
                                        tempRoot = roots[i];
                                    }
                                    else {
                                        initRoot = roots[i];
