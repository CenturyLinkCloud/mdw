...
public class PluginMessages {
    ...
    public static void uiMessage(Shell shell, String message, String title, WorkflowProject project,
            int level) {
        log(message);
    ...
    public static void uiMessage(Throwable t, String title, WorkflowProject workflowProject,
            int level) {
        if (getActiveWorkbenchWindow() != null)
    ...
    public static void uiMessage(Throwable t, String message, String title,
            WorkflowProject workflowProject, int level) {
        if (getActiveWorkbenchWindow() != null)
            uiMessage(getActiveWorkbenchWindow().getShell(), t, message, title, workflowProject,
                    level);
        else
    ...
    public static void uiMessage(String message, String title, WorkflowProject workflowProject,
            int level) {
        if (getActiveWorkbenchWindow() != null)
            uiMessage(getActiveWorkbenchWindow().getShell(), message, title, workflowProject,
                    level);
        else
            uiMessage((Shell) null, message, title, null, level);
    }
    ...
    public static void uiMessage(Shell shell, Throwable t, String title, WorkflowProject project,
            int level) {
        uiMessage(shell, t, null, title, project, level);
    }

    public static void uiMessage(Shell shell, Throwable t, String message, String title,
            WorkflowProject project, int level) {
        log(t);

        if (t instanceof ValidationException || t.getCause() instanceof ValidationException) {
            ValidationException valEx = t instanceof ValidationException ? (ValidationException) t
                    : (ValidationException) t.getCause();
    ...
    public static int uiList(Shell shell, String message, String title, List<?> items,
        ...
        if (items.size() > 0 && items.get(0) instanceof WorkflowElement) {
            ...
            for (Object item : items) {
                ...
                String label = (element instanceof WorkflowAsset
                        && element.getProject().checkRequiredVersion(5, 5) && flaggedItems == null)
                                ? element.getLabelWithPackage() : element.getLabel();
    ...
    public static int getReportingLevel() {
        int level = MdwPlugin.getDefault().getPreferenceStore()
                .getInt(PreferenceConstants.PREFS_MDW_REPORTING_LEVEL);
        if (level == 0)
    ...
    public static boolean isConnectionException(Exception ex) {
        ...
        if (userMsg != null && userMsg.indexOf("ORA-01017") >= 0)
            return true;
        if (userMsg != null
                && userMsg.indexOf("Network Adapter could not establish the connection") >= 0)
            return true;
    ...
    public static boolean isDataIntegrityException(Exception ex) {
        String userMsg = getUserMessage(ex);
        if (userMsg != null && (userMsg.indexOf("ORA-02292") >= 0
                || userMsg.indexOf("The process has instances") >= 0))
            return true;
