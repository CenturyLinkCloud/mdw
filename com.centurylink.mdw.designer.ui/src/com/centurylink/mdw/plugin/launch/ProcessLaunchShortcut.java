...
public class ProcessLaunchShortcut implements ILaunchShortcut {
    ...
    private void performLaunch(WorkflowProcess processVersion, String mode)
        ...
        ILaunchConfigurationWorkingCopy workingCopy = createLaunchConfiguration(
                PROCESS_LAUNCH_CONFIG_TYPE, processVersion, debug);
        ILaunchConfiguration config = findExistingLaunchConfiguration(workingCopy,
                processVersion.getProject(), debug);
        if (config == null) {
    ...
    protected ILaunchConfigurationWorkingCopy createLaunchConfiguration(String typeId,
            WorkflowProcess processVersion, boolean debug) throws CoreException {
        ...
        wc.setAttribute(ProcessLaunchConfiguration.PROCESS_VERSION,
                processVersion.getVersionString());
        ...
        if (debug) {
            ...
            wc.setAttribute(IJavaLaunchConfigurationConstants.ATTR_CONNECT_MAP,
                    getDefaultConnectArgs(workflowProject));
        }
        wc.setContainer(null);
        return wc;
    }

    protected String getUniqueLaunchConfigName(WorkflowProcess processVersion)
            throws CoreException {
        String name = processVersion.getName();
        ILaunchConfigurationType configType = getLaunchManager()
                .getLaunchConfigurationType(PROCESS_LAUNCH_CONFIG_TYPE);
    ...
    protected ILaunchConfiguration findExistingLaunchConfiguration(
            ILaunchConfigurationWorkingCopy workingCopy, WorkflowProject workflowProject,
            boolean debug) throws CoreException {
        String wcWorkflowProj = workingCopy
        ...
        for (ILaunchConfiguration launchConfig : configs) {
            ...
            String processVersion = launchConfig
                    .getAttribute(ProcessLaunchConfiguration.PROCESS_VERSION, "");
            if (!wcProcessVersion.equals(processVersion)) {
                ...
                updatedWc.setAttribute(ProcessLaunchConfiguration.PROCESS_VERSION,
                        wcProcessVersion);
            ...
            if (debug) {
                ...
                if (debugProj.isEmpty()) {
                    ...
                    updatedWc.setAttribute(IJavaLaunchConfigurationConstants.ATTR_PROJECT_NAME,
                            workflowProject.getSourceProject().getName());
                }
                if (vmConnector.isEmpty()) {
                    if (updatedWc == null)
                        updatedWc = launchConfig.getWorkingCopy();
                    updatedWc.setAttribute(IJavaLaunchConfigurationConstants.ATTR_VM_CONNECTOR,
                            "org.eclipse.jdt.launching.socketAttachConnector");
                }
                if (connectAttrMap.isEmpty()) {
                    if (updatedWc == null)
                        updatedWc = launchConfig.getWorkingCopy();
                    updatedWc.setAttribute(IJavaLaunchConfigurationConstants.ATTR_CONNECT_MAP,
                            getDefaultConnectArgs(workflowProject));
                }
                if (updatedWc != null)
                    launchConfig = updatedWc.doSave();
    ...
    protected Map<String, String> getDefaultConnectArgs(WorkflowProject workflowProject) {
        ...
        argDefaults.put("hostname", workflowProject.isRemote()
                ? workflowProject.getServerSettings().getHost() : "localhost");
