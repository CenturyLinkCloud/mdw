...
public abstract class WorkflowLaunchConfiguration extends LaunchConfigurationDelegate
{
  ...
  public boolean isWriteToConsole() { return writeToConsole; }
  public void setWriteToConsole(boolean write) { writeToConsole = write; }

  public void fireExternalEvent(WorkflowProject workflowProject, String request, Map<String,String> headers)
  {
    try
    {
      String response = workflowProject.getDesignerProxy().sendExternalEvent(request, headers);
      if (writeToConsole)
  ...
  protected void showError(final Exception ex, final String title, final WorkflowProject workflowProject)
  {
    MdwPlugin.getDisplay().asyncExec(new Runnable()
    {
      public void run()
