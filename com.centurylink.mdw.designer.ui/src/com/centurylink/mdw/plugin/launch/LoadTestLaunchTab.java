...
public class LoadTestLaunchTab extends TestSuiteLaunchTab
{
  ...
  public void initializeFrom(ILaunchConfiguration launchConfig)
  {
    ...
    try
    {
      int threadInterval = launchConfig.getAttribute(getAttrPrefix() + AutomatedTestLaunchConfiguration.RUN_INTERVAL, 2);
      runIntervalSpinner.setSelection(threadInterval);

      Map<String,String> testCaseCountsStrMap = launchConfig.getAttribute(
          getAttrPrefix() + AutomatedTestLaunchConfiguration.TESTCASE_COUNTS_MAP, new HashMap<String,String>());
      for (Object key : testCaseCountsStrMap.keySet())
        testCaseCounts.put(key.toString(), Integer.parseInt(testCaseCountsStrMap.get(key).toString()));
  ...
  public void performApply(ILaunchConfigurationWorkingCopy launchConfig)
  {
    ...
    launchConfig.setAttribute(getAttrPrefix() + AutomatedTestLaunchConfiguration.RUN_INTERVAL, runIntervalSpinner.getSelection());
    ...
    launchConfig.setAttribute(getAttrPrefix() + AutomatedTestLaunchConfiguration.TESTCASE_COUNTS_MAP, testCaseCountsStrMap);
  }

  public void setDefaults(ILaunchConfigurationWorkingCopy launchConfig)
  {
    launchConfig.setAttribute(getAttrPrefix() + AutomatedTestLaunchConfiguration.RUN_INTERVAL, 2);
  }
  ...
  protected void validatePage()
  {
    ...
    if (getProject() == null ||
        (!getProject().checkRequiredVersion(5, 2) && !getProject().checkRequiredVersion(4, 5, 20)))
    {
      setErrorMessage("Load Testing requires a Workflow Project with an MDW Version of at least 5.2 or 4.5.20");
    }
