...
public class ProcessLaunchMainTab extends AbstractLaunchConfigurationTab {
    ...
    private Image serverStartedImage = MdwPlugin.getImageDescriptor("icons/server_started.gif")
            .createImage();
    private Image serverStoppedImage = MdwPlugin.getImageDescriptor("icons/stopped.gif")
            .createImage();
    ...
    private Image liveViewImage = MdwPlugin.getImageDescriptor("icons/display_prefs.gif")
            .createImage();
    ...
    public void initializeFrom(ILaunchConfiguration launchConfig) {
        try {
            ...
            if (procVer.length() > 0) {
                ...
                ProcessVO processVO = project.getDesignerProxy()
                        .getProcessVO(processNameCombo.getText(), procVer);
            ...
            if (execModeAsyncButton != null) {
                boolean synchronous = launchConfig
                        .getAttribute(ProcessLaunchConfiguration.SYNCHRONOUS, false);
                execModeAsyncButton.setSelection(!synchronous);
                execModeSyncButton.setSelection(synchronous);

                String responseVarName = launchConfig
                        .getAttribute(ProcessLaunchConfiguration.RESPONSE_VAR_NAME, "");
                responseVarNameCombo.setText(responseVarName);
                responseVarNameCombo.setEnabled(synchronous);
            }

            boolean showLogs = launchConfig.getAttribute(ProcessLaunchConfiguration.SHOW_LOGS,
                    false);
            showLogOutputCheckbox.setSelection(showLogs);

            int logWatcherPort = launchConfig.getAttribute(
                    ProcessLaunchConfiguration.LOG_WATCHER_PORT,
                    project.getServerSettings().getLogWatcherPort());
            logWatcherPortText.setText(String.valueOf(logWatcherPort));

            if (showLogs) {
                boolean liveView = launchConfig.getAttribute(ProcessLaunchConfiguration.LIVE_VIEW,
                        false);
    ...
    public void performApply(ILaunchConfigurationWorkingCopy launchConfig) {
        launchConfig.setAttribute(ProcessLaunchConfiguration.WORKFLOW_PROJECT,
                workflowProjectCombo.getText());
        launchConfig.setAttribute(ProcessLaunchConfiguration.PROCESS_NAME,
                processNameCombo.getText());
        launchConfig.setAttribute(ProcessLaunchConfiguration.PROCESS_VERSION,
                processVersionCombo.getText());
        launchConfig.setAttribute(ProcessLaunchConfiguration.MASTER_REQUEST_ID,
                masterRequestIdText.getText().trim());
        ...
        if (execModeSyncButton != null) {
            launchConfig.setAttribute(ProcessLaunchConfiguration.SYNCHRONOUS,
                    execModeSyncButton.getSelection());
            launchConfig.setAttribute(ProcessLaunchConfiguration.RESPONSE_VAR_NAME,
                    responseVarNameCombo.getText());
        }
        launchConfig.setAttribute(ProcessLaunchConfiguration.SHOW_LOGS,
                showLogOutputCheckbox.getSelection());
        try {
        ...
        launchConfig.setAttribute(ProcessLaunchConfiguration.LIVE_VIEW,
                liveViewCheckbox.getSelection());
    }
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (version.length() == 0) {
                    ...
                    if (name.length() == 0) {
                        ...
                        ProcessVO processVO = project.getDesignerProxy().getProcessVO(name,
                                version);
                        if (processVO != null) {
                            process = new WorkflowProcess(project, processVO);
                            openProcess(process);
                ...
                if (status.equals(SERVER_STATUS_WAIT)) {
                    ...
                    if (msg == null) {
                        ...
                        if (!getProject().isFilePersist()) {
                            ...
                            setErrorMessage(
                                    "Please ensure that the server database configuration matches the workflow project.");
                        }
                    }
                }
