...
public class ProcessLaunchConfiguration extends WorkflowLaunchConfiguration
{
  ...
  public void launch(ILaunchConfiguration launchConfig, String mode, ILaunch launch, IProgressMonitor monitor) throws CoreException
  {
    ...
    {
      showError("Can't locate workflow project: '" + wfProjectName + "'.", "Process Launch", null);
      return;
    }
    ...
    if (process == null)
    {
      ...
      if (procVO == null)
      {
        showError("Can't locate process '" + processName + " v" + processVersion + "' in " + wfProjectName + ".", "Process Launch", workflowProject);
        return;
      }
      else
      {
    ...
    launchProcess(process, masterRequestId, owner, new Long(ownerId), synchronous, responseVarName, variableValues, null, showLogs, logWatchPort, liveView);
  }
  ...
  private boolean connectForDebug(WorkflowProject workflowProject, ILaunchConfiguration launchConfig)
  {
    try
    {
      ILaunchConfigurationWorkingCopy workingCopy = createDebugLaunchConfig(workflowProject, launchConfig);
  ...
  private ILaunchConfiguration findExistingDebugLaunchConfig(ILaunchConfigurationWorkingCopy workingCopy) throws CoreException
  {
    ...
    for (ILaunchConfiguration launchConfig : configs)
    {
      ...
      {
        if (!"timeout".equals(key) && !connectAttrMap.get(key).equals(wcConnectAttrMap.get(key)))
        {
          mapMatches = false;
          continue;
        }
