...
public class AssetSearchQuery extends SearchQuery {
    ...
    public void setContainedText(String text) {
        this.containedText = text;
    }

    public AssetSearchQuery(List<WorkflowProject> scopedProjects, SearchType searchType,
    ...
    public IStatus run(IProgressMonitor monitor) throws OperationCanceledException {
        ...
        for (WorkflowProject project : getScopedProjects()) {
            if (getSearchType().equals(SearchType.ENTITY_BY_NAME)
                    || (getSearchType().equals(SearchType.ENTITY_BY_ID)
                            && getPattern().equals("*"))) {
                if (getResourceType().equalsIgnoreCase("External Event Handler")) {
                    for (ExternalEvent eventHandler : project.getAllExternalEvents()) {
                        String name = isCaseSensitive() ? eventHandler.getName()
                                : eventHandler.getName().toLowerCase();
                        if ((getPattern().equals("*") || name.indexOf(getPattern()) >= 0)
                                && (getSelectedPackage() == null
                                        || (eventHandler.getPackage() != null && eventHandler
                                                .getPackage().equals(getSelectedPackage()))))
                    ...
                    for (WorkflowAsset asset : project.getAllWorkflowAssets()) {
                        if (asset.getLanguage().equalsIgnoreCase(getResourceType())) {
                            String name = isCaseSensitive() ? asset.getName()
                                    : asset.getName().toLowerCase();
                            if ((getPattern().equals("*") || name.indexOf(getPattern()) >= 0)
                                    && (getSelectedPackage() == null || (asset.getPackage() != null
                                            && asset.getPackage().equals(getSelectedPackage()))))
                    ...
                    for (WorkflowAsset asset : project.getAllWorkflowAssets()) {
                        if (asset.getLanguage().equalsIgnoreCase(getResourceType())) {
                            ...
                            if ((getPattern().equals("*") || name.indexOf(getPattern()) >= 0)
                                ...
                                // resource name pattern is matched, check for
                                // contained text
                                asset = asset.getProject().getDesignerProxy()
                                        .loadWorkflowAsset(asset);
                                if (!asset.isBinary()) {
                                    String content = isCaseSensitive() ? asset.getContent()
                                            : asset.getContent().toLowerCase();
                                    String searchText = isCaseSensitive() ? getContainedText()
                                            : getContainedText().toLowerCase();
                                    searchText = searchText.replaceAll("\\*", "");
    ...
    private void openExternalEvent(ExternalEvent eventHandler) {
        try {
            ...
            PluginMessages.uiError(MdwPlugin.getShell(), ex, "Event Handler",
                    eventHandler.getProject());
        }
    }
}
