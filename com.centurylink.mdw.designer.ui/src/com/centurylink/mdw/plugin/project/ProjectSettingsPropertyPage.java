...
public class ProjectSettingsPropertyPage extends ProjectPropertyPage {
    ...
    private void createRelatedProjectControls(Composite parent) {
        ...
        int style = (sourceProject == null || sourceProject.length() == 0) ? SWT.SINGLE | SWT.BORDER
    ...
    private void createJdbcUrlControls(Composite parent) {
        ...
        databaseJdbcUrlTextField.addModifyListener(new ModifyListener() {
            public void modifyText(ModifyEvent e) {
                boolean success = getProject().getMdwDataSource()
                        .setJdbcUrlWithCredentials(databaseJdbcUrlTextField.getText().trim());
                setValid(success);
    ...
    private void createGitRepositoryControls(Composite parent) {
        ...
        gitRepositoryUrlTextField.addModifyListener(new ModifyListener() {
            public void modifyText(ModifyEvent e) {
                boolean success = getProject().getMdwVcsRepository()
                        .setRepositoryUrlWithCredentials(
                                gitRepositoryUrlTextField.getText().trim());
                setValid(success);
        ...
        assetLocalPathTextField.addModifyListener(new ModifyListener() {
            public void modifyText(ModifyEvent e) {
                getProject().getMdwVcsRepository()
                        .setLocalPath(assetLocalPathTextField.getText().trim());
            }
        });
        ...
        if (getProject().isRemote()) {
            ...
            includeArchiveCheckbox.addSelectionListener(new SelectionAdapter() {
                public void widgetSelected(SelectionEvent e) {
                    getProject().getMdwVcsRepository()
                            .setSyncAssetArchive(includeArchiveCheckbox.getSelection());
                }
            });
        ...
        portTextField.addModifyListener(new ModifyListener() {
            public void modifyText(ModifyEvent e) {
                getProject().getServerSettings()
                        .setPort(Integer.parseInt(portTextField.getText().trim()));
            }
        });
    ...
    public boolean performCancel() {
        ...
        if (getProject().getPersistType() == PersistType.Git) {
            if (originalGitRepositoryUrl != null)
                getProject().getMdwVcsRepository()
                        .setRepositoryUrlWithCredentials(originalGitRepositoryUrl);
    ...
    public boolean performOk() {
        ...
        if (mdwVersionComboBox != null
                && !mdwVersionComboBox.getText().equals(originalMdwVersion)) {
            getProject().fireElementChangeEvent(ChangeType.VERSION_CHANGE,
                    getProject().getMdwVersion());
            if (MessageDialog.openQuestion(getShell(), "Update Framework Libraries",
                ...
                try {
                    ...
                    ExtensionModulesUpdater modulesUpdater = new ExtensionModulesUpdater(
                            getProject());
                    modulesUpdater.doUpdate(getShell());
                }
                catch (Exception ex) {
                    PluginMessages.uiError(getShell(), ex, "Update Framework Libraries",
                            getProject());
                    return false;
                }
            }
        ...
        if (updateServerCacheCheckbox.getSelection() != originalRefreshServerCache)
            getProject().fireElementChangeEvent(ChangeType.SETTINGS_CHANGE,
                    getProject().isUpdateServerCache());
