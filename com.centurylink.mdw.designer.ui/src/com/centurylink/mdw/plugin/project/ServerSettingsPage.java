...
public abstract class ServerSettingsPage extends WizardPage implements IFacetWizardPage {
    ...
    public void initValues() {
        ...
        if (type != null) {
            String prevServerHome = MdwPlugin
                    .getStringPref(type + "-" + ProjectPersist.MDW_SERVER_HOME);
            ...
            String prevJdkHome = MdwPlugin
                    .getStringPref(type + "-" + ProjectPersist.MDW_SERVER_JDK_HOME);
            ...
            String prevServerHost = MdwPlugin
                    .getStringPref(type + "-" + ProjectPersist.MDW_SERVER_HOST);
            ...
            String prevServerPort = MdwPlugin
                    .getStringPref(type + "-" + ProjectPersist.MDW_SERVER_PORT);
            if (prevServerPort.length() > 0 && getProject().isRemote()) {
                try {
                    getServerSettings().setPort(Integer.parseInt(prevServerPort));
                }
                catch (NumberFormatException ex) {
                    MdwPlugin.setStringPref(type + "-" + ProjectPersist.MDW_SERVER_PORT,
            ...
            String prevServerUser = MdwPlugin
                    .getStringPref(type + "-" + ProjectPersist.MDW_SERVER_USER);
        ...
        if (!getProject().checkRequiredVersion(5, 0)) {
            IProjectFacetVersion pfv = ProjectFacetsManager.getProjectFacet("mdw.workflow")
                    .getVersion("1.1");
            IFacetedProjectWorkingCopy wc = ((ModifyFacetedProjectWizard) getWizard())
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (!checkString(serverHome)) {
                    MessageDialog.openError(getShell(), "Missing Data",
                            "Please enter a value for " + getServerName() + " Home.");
                }
                else {
    ...
    public IStatus[] getStatuses() {
        ...
        if (!checkDir(serverHomeTextField.getText().trim()))
        ...
        else if (serverSettings.getJdkHome() != null && serverSettings.getJdkHome().length() != 0
                && !checkFile(serverSettings.getJdkHome() + "/jre/lib/rt.jar"))
            ...
            msg = "Please enter a valid directory for " + getServerName() + " "
                    + getServerLocationLabel();
        else if (containsWhitespace(serverSettings.getServerLoc()))
            msg = getServerName() + " " + getServerLocationLabel()
    ...
    public IWizardPage getNextPage() {
        ...
        for (IWizardPage page : getWizard().getPages()) {
            ...
            else if (persistType == PersistType.Git
                    && page.getTitle().equals(GitRepositoryPage.PAGE_TITLE)) {
