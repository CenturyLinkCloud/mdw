...
public class ServiceMixSettingsPage extends ServerSettingsPage
{
  ...
  protected Button getUseRootInstanceCheckbox() { return useRootInstanceCheckbox; }
  ...
  public String getServerName() { return "ServiceMix"; }
  public int getDefaultServerPort() { return DEFAULT_HTTP_PORT; }
  public int getDefaultServerSshPort() { return DEFAULT_SSH_PORT; }
  ...
  protected void createServerPortControls(Composite parent, int ncol)
  {
    ...
    serverSshPortTextField.addModifyListener(new ModifyListener()
    {
      public void modifyText(ModifyEvent e)
      {
        getServerSettings().setCommandPort(Integer.parseInt(serverSshPortTextField.getText().trim()));
        handleFieldChanged();
      }
    });
  }
  ...
  protected String serverHomeSpecializedCheck(String serverHome)
  {
    ...
    String msg = null;
    if (serverHome != null && serverHome.length() != 0 && !(checkFile(serverHome + "/bin/servicemix.bat") || checkFile(serverHome + "/bin/servicemix.sh")))
  ...
  {
    String msg = null;
    if (serverLoc != null && serverLoc.length() != 0 && !(checkFile(serverLoc + "/bin/karaf.bat") || checkFile(serverLoc + "/bin/karaf.sh")))
