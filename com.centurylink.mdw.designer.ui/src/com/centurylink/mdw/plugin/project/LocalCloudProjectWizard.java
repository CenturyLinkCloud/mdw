...
public class LocalCloudProjectWizard extends Wizard implements INewWizard {
    ...
    public WorkflowProjectPage getWorkflowProjectPage() {
        return workflowProjectPage;
    }

    private WebLogicSettingsPage webLogicSettingsPage;

    public WebLogicSettingsPage getWebLogicSettingsPage() {
        return webLogicSettingsPage;
    }

    private JBossSettingsPage jbossSettingsPage;

    public JBossSettingsPage getJBossSettingsPage() {
        return jbossSettingsPage;
    }

    private ServiceMixSettingsPage serviceMixSettingsPage;

    public ServiceMixSettingsPage getServiceMixSettingsPage() {
        return serviceMixSettingsPage;
    }

    private FuseSettingsPage fuseSettingsPage;

    public FuseSettingsPage getFuseSettingsPage() {
        return fuseSettingsPage;
    }

    private TomcatSettingsPage tomcatSettingsPage;

    public TomcatSettingsPage getTomcatSettingsPage() {
        return tomcatSettingsPage;
    }

    private ExtensionModulesWizardPage extensionModulesPage;

    public ExtensionModulesWizardPage getExtensionModulesPage() {
        return extensionModulesPage;
    }
    ...
    public GitRepositoryPage getGitRepositoryPage() {
    ...
    public boolean canFinish() {
        return workflowProjectPage.isPageComplete()
                && ((project.getServerSettings().isJBoss() && jbossSettingsPage.isPageComplete())
                        || (project
                                .getServerSettings().isServiceMix()
                                && serviceMixSettingsPage.isPageComplete())
                        || (project.getServerSettings().isFuse()
                                && fuseSettingsPage.isPageComplete())
                        || (project.getServerSettings().isWebLogic()
                                && webLogicSettingsPage.isPageComplete())
                        || (project.getServerSettings().isTomcat()
                                && tomcatSettingsPage.isPageComplete()))
                && (project.getPersistType() == PersistType.None
                        || (project.getPersistType() == PersistType.Git
                                ? gitRepositoryPage.isPageComplete()
    ...
    public boolean performFinish() {
        project.setDefaultFilesToIgnoreDuringUpdate(); // can depend on
                                                       // container selected
        ...
        if (!project.isRemote() && !project.isWar()) {
            BusyIndicator.showWhile(getShell().getDisplay(), new Runnable() {
                public void run() {
                    // remove the pesky temp (or workflow) folder from the
                    ...
                    try {
                        ...
                        getProject().getSourceProject().refreshLocal(IResource.DEPTH_INFINITE,
                                new NullProgressMonitor());
                        if (getProject().isFilePersist())
                            projConf.removeDeploymentAssemblyResourceMappings(
                                    project.getAssetFolder());
                        else
                            projConf.removeDeploymentAssemblyResourceMappings(
                                    project.getTempFolder());
                        projConf.addDeploymentAssemblyResourceMappings(
                                project.getSourceProject().getFolder("/src/main/resources"));
                    }
