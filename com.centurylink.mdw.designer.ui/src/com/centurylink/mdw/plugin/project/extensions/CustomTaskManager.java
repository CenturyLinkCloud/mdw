...
public class CustomTaskManager extends ExtensionModule
{
  ...
  public String writeConfigElement(WorkflowProject project)
  {
    return "  <webProject name=\"" + project.getWebProjectName() + "\""
        + (project.getDeployDir() == null ? "" : " deployDir=\"" + project.getDeployDir() + "\"") + " />\n";
  }
  ...
  @Override
  public boolean removeFrom(WorkflowProject project, IProgressMonitor monitor) throws ExtensionModuleException
  {
    monitor.worked(5);

    try
    {
      updateApplicationXml(project, "MDWTaskManagerWeb.war", project.getSourceProjectName() + "TaskManager", new SubProgressMonitor(monitor, 10));
  ...
  {
    String message = "Custom Task Manager project references will be removed.  (Please delete web project '" + project.getWebProjectName() + "' by hand.)";
  ...
  private boolean launchWebProjectWizard(Shell shell, WorkflowProject workflowProject)
  {
    ...
    dataModel.setStringProperty(IFacetProjectCreationDataModelProperties.FACET_PROJECT_NAME, workflowProject.getWebProjectName());
    ...
    FacetDataModelMap facetDmMap = (FacetDataModelMap) webProjectWizard.getDataModel().getProperty(IFacetProjectCreationDataModelProperties.FACET_DM_MAP);
    // web facet data model props
    IDataModel webFacetDataModel = (IDataModel) facetDmMap.get("jst.web");
    webFacetDataModel.setStringProperty(IJ2EEModuleFacetInstallDataModelProperties.CONFIG_FOLDER, "web");

    if (dialog.open() != Dialog.OK)
      return false;

    // in case user changed the web project name
    String webProjectName = (String) webProjectWizard.getDataModel().getProperty(IFacetProjectCreationDataModelProperties.FACET_PROJECT_NAME);
