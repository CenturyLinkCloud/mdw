...
public class GradleBuildFile implements OsgiBuildFile {
    ...
    public OsgiManifestDescriptor parse() throws Exception {
        ...
        if (rootBuildFile != null && rootBuildFile.exists()) {
            ...
            if (rootBuildFileContent.indexOf("${") >= 0)
                rootBuildFileContent = ExpressionUtil.substitute(rootBuildFileContent, buildProps,
                        true);
        }
        ...
        // symbolicName, exportPackage, importPackage, privatePackage,
        // dynamicImportPackage, bundleClasspath, bundleActivator,
        // webContextPath
        ...
        manifestDescriptor = new OsgiManifestDescriptor(
                project.getFolder(outputDirectory).getLocation().toFile());
        ...
        manifestDescriptor
                .setDynamicImportPackage(findInstruction(mfLines, "DynamicImport-Package"));
    ...
    private String readBuildFileValue(String name, boolean includeRootBuildFile)
            throws MDWException {
        String begin = "\n\\s*" + name + "\\s*\\=";
        String end = "\n";
    ...
    private File getRootBuildFile() throws Exception {
        IFile prefsFile = project
                .getFile(".settings/gradle/org.springsource.ide.eclipse.gradle.core.prefs");
        if (prefsFile.exists()) {
            Properties prefsProps = new Properties();
            prefsProps.load(prefsFile.getContents());
            String rootLoc = (String) prefsProps
                    .get("org.springsource.ide.eclipse.gradle.rootprojectloc");
            if (rootLoc != null) {
