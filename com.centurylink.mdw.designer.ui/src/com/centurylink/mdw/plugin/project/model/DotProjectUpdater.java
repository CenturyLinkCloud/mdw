    ...
    for (int i = 0; i < projectChildren.getLength(); i++)
    {
      ...
      if ("natures".equals(projectChild.getNodeName()))
      {
        ...
        for (int j = 0; j < natureChildren.getLength(); j++)
        {
          ...
          if ("nature".equals(natureChild.getNodeName()) && (natureChild.getFirstChild() != null && natureChild.getFirstChild().getNodeType() == Node.TEXT_NODE) && nature.equals(natureChild.getFirstChild().getNodeValue()))
            found = true;
        }
        if (!found)
        {
