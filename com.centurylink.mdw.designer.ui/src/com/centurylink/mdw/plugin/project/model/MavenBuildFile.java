...
public class MavenBuildFile implements OsgiBuildFile
{
  ...
  public String getArtifactName()
  {
    ...
    return finalName.replace("${project.artifactId}", artifactId).replace("${project.version}", version);
  }
  ...
  public String getArtifactGenDir() { return appGeneratedDir; }
    ...
    parser.parse(src, new DefaultHandler()
    {
      ...
      public void characters(char[] ch, int start, int length) throws SAXException
      {
        if (inGroupIdElem)
          ...
          manifestDescriptor.setExportPackage(manifestDescriptor.getExportPackage() + new String(ch).substring(start, start + length).trim());
        else if (inPrivatePkgElem)
          manifestDescriptor.setPrivatePackage(manifestDescriptor.getPrivatePackage() + new String(ch).substring(start, start + length).trim());
        else if (inImportPkgElem)
          manifestDescriptor.setImportPackage(manifestDescriptor.getImportPackage() + new String(ch).substring(start, start + length).trim());
        else if (inDynamicImportPkgElem)
          manifestDescriptor.setDynamicImportPackage(manifestDescriptor.getDynamicImportPackage() + new String(ch).substring(start, start + length).trim());
        else if (inBundleActivatorElem)
          manifestDescriptor.setBundleActivator(manifestDescriptor.getBundleActivator() + new String(ch).substring(start, start + length).trim());
        else if (inBundleClasspathElem)
          manifestDescriptor.setBundleClasspath(manifestDescriptor.getBundleClasspath() + new String(ch).substring(start, start + length).trim());
        else if (inBsnElem)
          manifestDescriptor.setSymbolicName(manifestDescriptor.getSymbolicName() + new String(ch).substring(start, start + length).trim());
