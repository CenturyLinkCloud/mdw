...
public class ServerConnectionsPropertyPage extends ProjectPropertyPage
    ...
    private void createContainerControls(Composite parent) {
        ...
        browseServerHomeButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                dlg.setMessage(
                        "Select the directory where " + getContainerName() + " is installed.");
    ...
    private void createServerControls(Composite parent) {
        ...
        if (getProject().getServerSettings().isServiceMix()
                || getProject().getServerSettings().isFuse()) {
            ...
            serverCommandPortTextField.addModifyListener(new ModifyListener() {
                public void modifyText(ModifyEvent e) {
                    workingCopy.setCommandPort(
                            Integer.parseInt(serverCommandPortTextField.getText().trim()));
                }
            });
        ...
        if (!getProject().isWar()) {
            new Label(composite, SWT.NONE)
                    .setText(workingCopy.isWebLogic() ? "Domain Location:" : "Server Location:");
            ...
            serverLocTextField.addModifyListener(new ModifyListener() {
                public void modifyText(ModifyEvent e) {
                    ...
                    try {
                        ...
                        if (serverMessageLabel != null && "Server appears to be running."
                                .equals(serverMessageLabel.getText())) {
            ...
            browseServerLocButton.addSelectionListener(new SelectionAdapter() {
                public void widgetSelected(SelectionEvent e) {
                    DirectoryDialog dlg = new DirectoryDialog(getShell());
                    dlg.setMessage(
                            "Select the root directory of your " + getContainerName() + " Server.");
            ...
            newServerButton.addSelectionListener(new SelectionAdapter() {
                public void widgetSelected(SelectionEvent e) {
                    ...
                    if (serverHome == null || serverHome.trim().length() == 0) {
                        MessageDialog.openError(getShell(), "Missing Data",
                                "Please enter a value for " + getContainerName() + " Home.");
                    }
                    else {
    ...
    private void createDeployControls(Composite parent) {
        ...
        deleteTempFilesCheckbox.setEnabled(false);
        MdwPlugin.setStringPref(PreferenceConstants.PREFS_DELETE_TEMP_FILES_AFTER_SERVER_CONFIG,
                Boolean.TRUE.toString());
        deleteTempFilesCheckbox.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                MdwPlugin.setStringPref(
                        PreferenceConstants.PREFS_DELETE_TEMP_FILES_AFTER_SERVER_CONFIG,
                        String.valueOf(deleteTempFilesCheckbox.getSelection()));
            }
        });
    }
        ...
        MdwPlugin.getDisplay().syncExec(new Runnable() {
            public void run() {
                ...
                if (status.equals(SERVER_STATUS_WAIT)) {
                    ...
                    if (!PARSE_ERROR.equals(getErrorMessage()) && serverLocTextField != null
                            && serverLocTextField.getText().trim().length() > 0) {
                        if (startServerButton != null)
                            startServerButton.setEnabled(false);
                        if (deployAppButton != null)
                            deployAppButton.setEnabled(true);
                    ...
                    serverMessageLabel.setText("Server not reachable at: "
                            + getProject().getServerSettings().getConsoleUrl() + ".");
                    if (deployAppButton != null)
                        deployAppButton.setEnabled(false);
                    if (deleteTempFilesCheckbox != null)
                        deleteTempFilesCheckbox.setEnabled(false);
                    ...
                    serverMessageLabel.setText(
                            "Unexpected error accessing the server.  See .metadata/.log for details.");
                    if (deployAppButton != null)
                        deployAppButton.setEnabled(false);
                    if (deleteTempFilesCheckbox != null)
                        deleteTempFilesCheckbox.setEnabled(false);
