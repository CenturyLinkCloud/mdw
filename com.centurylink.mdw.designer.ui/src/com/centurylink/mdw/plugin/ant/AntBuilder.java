...
public class AntBuilder {
    ...
    public String getBuildFileLoc() {
        return buildFileLoc;
    }

    public void setBuildFileLoc(String loc) {
        buildFileLoc = loc;
    }
    ...
    public String getBuildErrorMessages() {
    ...
    public AntBuilder(File buildFile) {
        ...
        MessageConsole messageConsole = MessageConsole.findConsole("Server Config",
                MdwPlugin.getImageDescriptor("icons/ant.gif"), MdwPlugin.getDisplay());
        ...
        getAntProject().addBuildListener(new DefaultLogger() {
            ...
            public void messageLogged(BuildEvent event) {
                ...
                if (event.getPriority() < Project.MSG_WARN
                        && !event.getMessage().startsWith("JAVA_HOME is set to")
                        && !event.getMessage().startsWith("USER_NAME is set to")) {
    ...
    public void build(IProgressMonitor monitor)
        ...
        for (Iterator<?> iter = ProjectHelperRepository.getInstance().getHelpers(); iter
                .hasNext();) {
