    ...
    protected FormDataDocument createTask(FormDataDocument formdatadoc, String content, Map<String, String> metainfo) {
        ...
        try {
            ...
            if (isDetailOnly) {
                ...
                int overriddenDetailDueInSeconds = 0;
                if (dueInSeconds != null && TaskManagerAccess.getInstance().isRemoteDetail()) {
                    int summaryDueInSecs = taskVo.getSlaSeconds();
                    if (summaryDueInSecs > 0) {
                        // we'll not override due dates configured in summary template -- just warn
                        overriddenDetailDueInSeconds = Integer.parseInt(dueInSeconds);
                        dueInSeconds = null;
                    }
                }
                ...
                if (overriddenDetailDueInSeconds != 0) {
                    Date summaryDueDate = instance.getDueDate();
                    if (summaryDueDate != null) {
                        Date detailDueDate = new Date(DatabaseAccess.getCurrentTime() + overriddenDetailDueInSeconds* 1000);
                        String msg = "Warning: Task Instance " + instance.getTaskInstanceId()
                                + " has due date from detail (" + detailDueDate
                                + ") that is being overridden by template SLA in summary (" + summaryDueDate + ")";
