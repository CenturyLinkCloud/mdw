            ...
            if (packageVO == null) {
                ...
                try {
                    ...
                    if (ApplicationContext.isOsgi()) {
                        // not local; try providers
                        TextService service = null;
                        if (Format.json.toString().equals(format)) {
                            service = MdwServiceRegistry.getInstance().getJsonService(serviceClassName);
                            if (service == null) { // try compatibility package for old taskmgr services
                                packageVO = PackageVOCache.getPackageVO("com.centurylink.mdw.taskmgr.services");
                                if (packageVO != null) {
                                    String compatServiceClassName = packageVO.getName() + "." + logicalName;
                                    service = MdwServiceRegistry.getInstance().getDynamicService(packageVO, JsonService.class, compatServiceClassName);
                                }
                            }
                        }
                        else {
                            service = MdwServiceRegistry.getInstance().getXmlService(serviceClassName);
                            if (service == null) { // try compatibility package for old taskmgr services
                                packageVO = PackageVOCache.getPackageVO("com.centurylink.mdw.taskmgr.services");
                                if (packageVO != null) {
                                    String compatServiceClassName = packageVO.getName() + "." + logicalName;
                                    service = MdwServiceRegistry.getInstance().getDynamicService(packageVO, XmlService.class, compatServiceClassName);
                                }
                            }
                        }
                        if (service == null)
