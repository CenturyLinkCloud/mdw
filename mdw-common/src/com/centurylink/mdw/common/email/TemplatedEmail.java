...
public class TemplatedEmail {
    ...
    private String getProperty(String propname) throws MessagingException {
        ...
        try {
            PackageVO pkg = PackageVOCache.getRuleSetPackage(getTemplate().getAssetId());
            if (pkg == null)
                return PropertyManager.getProperty(propname);
            else
        ...
        if (!images.isEmpty()) {
            for (String imageId : images.keySet()) {
                ...
                if (url == null) {
                    ...
                    final RuleSetVO imageRuleSet = RuleSetCache.getRuleSet(imageFile, "IMAGE_" + imageFile.substring(imageFile.lastIndexOf('.') + 1).toUpperCase());
                    if (imageRuleSet == null)
                        throw new MessagingException("Image not found: " + imageFile);
                    imageDataSource = new DataSource(){
                        public String getContentType() {
                            return imageRuleSet.getContentType();
                        }
                        public InputStream getInputStream() throws IOException {
                            byte[] bytes = imageRuleSet.getContent();
                            return new ByteArrayInputStream(bytes);
                        }
                        public String getName() {
                            return imageRuleSet.getName();
                        }
