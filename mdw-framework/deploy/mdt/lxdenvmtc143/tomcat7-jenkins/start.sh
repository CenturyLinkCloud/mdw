...
CATALINA_BASE=/prod/ecom2/local/apps/tomcat/tomcat7-jenkins

JAVA_OPTS="${JAVA_OPTS} -server -DJENKINS_HOME=/prod/ecom2/local/apps/jenkins -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -Xms128m -Xmx768m -XX:MaxPermSize=128m -XX:+UseParallelGC -XX:+UseParallelOldGC -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${CATALINA_BASE}/logs -XX:ErrorFile=${CATALINA_BASE}/logs/hs_err_pid%p.log -XX:+PrintGCDetails -Xloggc:${CATALINA_BASE}/logs/gclogs.log"
