...
public class GroovyTestCaseRun extends TestCaseRun {
    ...
    List<ProcessInstanceVO> loadProcess(TestCaseProcess[] processes, TestCaseAsset expectedResults) throws TestException {
        ...
        try {
            ...
            processInstances = loadResults(processVos, expectedResults);
    ...
    boolean verifyProcess(TestCaseProcess[] processes, TestCaseAsset expectedResults) throws TestException {
        ...
        try {
            ...
            if (getTestCase().isLegacy()) {
                processInstances = loadResults(processVos, null);
                return legacyVerifyProcesses();
            }
            else {
                processInstances = loadResults(processVos, expectedResults);
                if (processInstances.isEmpty())
                    throw new IllegalStateException("No process instances found");
        ...
        if (request != null && request.startsWith("{\"ActivityRuntimeContext\":")) {
            if (testCaseProcess != null && testCaseProcess.getActivityStubs() != null) {
                for (TestCaseActivityStub activityStub : testCaseProcess.getActivityStubs()) {
                    try {
                        ActivityRuntimeContext context = new ActivityRuntimeContext(new JSONObject(request));
                        if (activityStub.getMatcher().call(context)) {
                            ...
                            if (verbose)
                                log.println("Stubbing activity " + context.getProcess().getProcessName() + ":" +
                                        context.getActivityLogicalId() + " with result code: " + resultCode);
                            return "RESPONSE~" + activityStubResponse.getSleep() + "~" + activityStubResponse.getJson();
                        }
                    }
                    catch (JSONException ex) {
                        log.println("Error invoking activityStub: " + ex.getMessage());
            ...
            for (TestCaseAdapterStub adapterStub : adapterStubs) {
                if (adapterStub.getMatcher().call(request)) {
                    String stubbedResponse = adapterStub.getResponder().call(request);
                    if (verbose)
                        log.println("Stubbing response with: " + stubbedResponse);
                    return "RESPONSE~" + adapterStub.getDelay() + "~" + stubbedResponse;
                }
            }
            if (verbose)
