...
public abstract class GroovyTestCaseScript extends Script {
    ...
    public Verifiable verify(TestCaseResponse response) throws TestException {
        if (!getTestCaseRun().isLoadTest) {
            if (response.getExpected() == null) {
                // verification postponed until response.file() call for elegant command chaining
                this.response = new TestCaseResponse(response) {
                    @Override
                    public TestCaseResponse file(String name) throws TestException {
                        try {
                            String expected = substitute(new String(getTestCaseRun().read(GroovyTestCaseScript.this.file(name))));
                            GroovyTestCaseScript.this.response.setExpected(expected);
                            GroovyTestCaseScript.this.response.setSuccess(getTestCaseRun().verifyResponse(GroovyTestCaseScript.this.response));
                            return GroovyTestCaseScript.this.response;
                        }
                        catch (IOException ex) {
                            throw new TestException(ex.getMessage(), ex);
                        }
                    ...
                    public TestCaseResponse asset(String name) throws TestException {
                        try {
                            String expected = substitute((GroovyTestCaseScript.this.asset(name).text()));
                            GroovyTestCaseScript.this.response.setExpected(expected);
                            GroovyTestCaseScript.this.response.setSuccess(getTestCaseRun().verifyResponse(GroovyTestCaseScript.this.response));
                            return GroovyTestCaseScript.this.response;
                        }
