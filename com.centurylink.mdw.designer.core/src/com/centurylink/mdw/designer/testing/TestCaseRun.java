...
public class TestCaseRun extends ControlCommandShell implements Threadable {
            ...
            if (e.getCause() instanceof TestFailedException) {
                ...
                for (StackTraceElement el : e.getStackTrace()) {
                    if (el.getFileName() != null && el.getFileName().endsWith(".groovy")) {
                        log.println(" --> at " + getTestCase().getCaseFile().getName() + ":" + el.getLineNumber());
                    }
                }
            }
            else {
    ...
    private String translateToYaml(Map<String,List<ProcessInstanceVO>> processInstances, Map<String,String> activityNames, boolean orderById, String newLineChars)
    throws IOException, DataAccessException {
        ...
        for (String procName : processInstances.keySet()) {
            ...
            for (int i = 0; i < procInsts.size(); i++) {
                ...
                LinkedList<ActivityInstanceVO> orderedList = new LinkedList<ActivityInstanceVO>();
                for (ActivityInstanceVO act : procInst.getActivities())
    ...
    protected List<ProcessInstanceVO> loadResults(List<ProcessVO> processes, TestCaseAsset expectedResults, boolean orderById)
    ...
    throws TestException,IOException,DataAccessException,ParseException {
        ...
        loadResults(processes, null, false);
