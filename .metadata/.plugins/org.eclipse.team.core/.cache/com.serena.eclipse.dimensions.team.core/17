...
public class DesignerProxy
{
      ...
      if (project.getPersistType() == WorkflowProject.PersistType.Git)
      {
        ...
        if (project.isRemote())
        {
          try
          {
            ...
            {
              // non-git interim solution: download Packages zip from server
              ...
              // retrieve packages zip file
              String servicePath = "Packages?format=json&topLevel=true";
              if (gitRepo.isSyncAssetArchive())
                servicePath += "&archive=true";
              String json = restfulServer.invokeResourceService(servicePath);
  ...
  public DocumentVO getDocument(DocumentReference docRef)
  {
    try
    {
      // TODO: use REST access for VCS-based assets
      RuntimeDataAccess rao = DataAccess.getRuntimeDataAccess(new DatabaseAccess(project.getMdwDataSource().getJdbcUrlWithCredentials()), null);
      return rao.getDocument(docRef.getDocumentId());
    }
    catch (Exception ex)
    {
      PluginMessages.log(ex);
